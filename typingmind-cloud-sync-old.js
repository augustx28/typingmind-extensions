...
- Native TypingMind sync coexistence hardening: exclusion patterns for TM sync-state keys + mass-deletion safety guard [2026-07-14]
- Modal fixes: higher stacking order above TypingMind menus, scroll-safe centering on phones, self-contained styling for provider help guides (Google Client ID steps) [2026-07-15]
- Workspace bar: keep tab labels inside their own buttons so a crowded bar (multiple extensions, e.g. Tweaks) does not overlap labels; the bar container is tagged .tcs-workspace-bar so the guard also reaches third-party tabs, and the loaded build version is printed to the console for cache checks [2026-07-15]
- Sync button now clones the native tab's live svg/span classes (faithful structural mirror; also fixes third-party extensions like Tweaks that use this button as their styling reference) [2026-07-15]
...
const TCS_BUILD_VERSION = "2026-07-15.4";
// Always announce the loaded build (independent of ?log=true) so stale
// cached copies of this extension are easy to spot from the console.
console.info(`[TCS] TypingMind Cloud Sync build ${TCS_BUILD_VERSION} loaded`);
...
if (window.typingMindCloudSync) {
  ...
  class ConfigManager {
    ...
    shouldExclude(key) {
      ...
      if (this.exclusions.includes(key)) return true;
      // Extension-internal, Google auth helper and debug console keys.
      if (
        key.startsWith("tcs_") ||
        key.startsWith("gsi_") ||
        key.includes("eruda")
      ...
      // TypingMind native-sync bookkeeping and per-device identity keys.
      // These must stay device-local: copying them between devices through
      // this extension can corrupt the official TypingMind Cloud Sync state.
      // The exact names TypingMind uses today (TM_useStateUpdateHistory,
      // TM_useLastVerifiedToken, INSTANCE_ID) are covered by the exact-match
      // list above; the patterns below also catch renamed or future variants
      // so this extension keeps working whether native sync is ON or OFF.
      if (/^TM_/.test(key) && /sync/i.test(key)) return true;
      if (/^INSTANCE_/i.test(key)) return true;
      return false;
    }
    reloadExclusions() {
      this.exclusions = this.loadExclusions();
  ...
  class DataService {
    ...
    async getAllItemKeys() {
      ...
      await new Promise((resolve, reject) => {
        ...
        // A silent partial key list here is dangerous: detectChanges() uses
        // this set to infer deletions, so an incomplete read used to be
        // interpreted as "the user deleted these items" and propagated as
        // tombstones. Fail loudly instead so the sync cycle aborts safely.
        request.onerror = () =>
          reject(
            new Error(
              "Failed to enumerate IndexedDB keys (aborting to protect against false deletion detection)"
            )
          );
      });
      for (let i = 0; i < localStorage.length; i++) {
  ...
  // via a single SHA-256 digest of the passphrase and caches it per session.
  // (NOTE: not PBKDF2 — do not change the derivation, or all existing
  // encrypted cloud data becomes undecryptable for current users.)
  ...
  // Wasabi, iDrive E2, GCS S3 API, etc.). Dynamically loads the AWS SDK v2
  ...
  value instanceof Uint8Array || value instanceof Blob
    ...
    : this.getItemSize(value);
      ...
      for (const itemId in this.metadata.items) {
        ...
        if (!localItemKeys.has(itemId) && !metadataItem.deleted) {
          deletionCandidates.push(itemId);
        }
      }

      // Mass-deletion safety guard [2026-07-14]:
      // If a very large share of known items suddenly appears "deleted", the
      // far more likely cause is an incomplete IndexedDB read or another sync
      // layer (e.g. TypingMind's native cloud sync) rewriting the database
      // mid-scan — not a real user action. Creating tombstones in that state
      // would propagate false deletions to every synced device. Skip tombstone
      // creation for this cycle; a later cycle with a consistent read will
      // still pick up any genuine deletions.
      const knownActiveCount = Object.values(this.metadata.items).filter(
        (m) => !m.deleted
      ).length;
      const massDeletionSuspected =
        deletionCandidates.length > 25 &&
        knownActiveCount > 0 &&
        deletionCandidates.length / knownActiveCount > 0.5;

      if (massDeletionSuspected) {
        this.logger.log(
          "warning",
          `🛑 Mass-deletion guard: ${deletionCandidates.length} of ${knownActiveCount} known items appear deleted locally. ` +
            `Skipping tombstone creation this cycle (likely an incomplete database read or another sync writing concurrently). ` +
            `If you really deleted this many items, use Force Export to push the change.`
        );
      } else {
        let newlyDeletedCount = 0;
        for (const itemId of deletionCandidates) {
          const metadataItem = this.metadata.items[itemId];
          this.logger.log(
            "info",
    ...
    async syncFromCloud() {
      ...
      try {
        ...
        // Presence-verified download guard [2026-07-15]: local metadata can
        // claim an item is already here while the item is physically absent
        // from IndexedDB/localStorage (e.g. TypingMind's native sync copying
        // this extension's tcs_local-metadata between devices, a cleared or
        // partial database, or an interrupted earlier download). The filter
        // below used to trust metadata alone, which made such items silently
        // un-downloadable forever. Verify against the keys that actually
        // exist locally and force a download for anything missing.
        const physicallyPresentKeys = await this.dataService.getAllItemKeys();
        ...
        const itemsToDownload = Object.entries(cloudMetadata.items).filter(
          ([key, cloudItem]) => {
            ...
            if (!physicallyPresentKeys.has(key)) {
              // Metadata says we have it, but the item is not actually in
              // this browser's storage - download it.
              return true;
            }
            if (!localItem) {
              return true;
    ...
    /**
     * Decodes a provider response Body into a UTF-8 string regardless of its
     * runtime type [2026-07-14]. AWS SDK v2 in the browser usually returns a
     * Buffer-like object whose toString() works, but some S3-compatible
     * services and the Google Drive provider can hand back plain Uint8Array,
     * ArrayBuffer, or string bodies. Calling .toString() on a plain typed
     * array yields comma-separated byte values ("123,34,...") which then
     * fails JSON.parse. This helper normalizes all cases.
     */
    _decodeBodyToString(body) {
      if (body == null) return "";
    ...
    async getCloudMetadataWithETag() {
      ...
      try {
        ...
        const bodyText = this._decodeBodyToString(result.Body).trim();
        if (!bodyText) {
          throw new Error(
            "Empty metadata.json received from cloud (aborting sync cycle)"
          );
        }
  ...
  class CloudSyncApp {
    ...
    insertSyncButton() {
      ...
      if (!document.getElementById("tcs-inline-style")) {
        ...
        /* Crowded-bar guard: when several extensions add workspace tabs
           (Sync, Tweaks, ...), a tab label that escapes its own button can
           overlap the neighbouring tab's label (e.g. "TweaksSettings").
           The Sync button tags the shared container with .tcs-workspace-bar
           so these rules reach every tab in the bar, including third-party
           tabs without workspace-tab-* element ids. position: relative
           re-anchors any absolutely-positioned label to its own tab instead
           of the whole bar; min-width: 0 lets flex items shrink instead of
           bleeding into neighbours. */
        .tcs-workspace-bar > button,
        .tcs-workspace-bar > a,
        .tcs-workspace-bar > div {
          min-width: 0;
          position: relative;
        }
        .tcs-workspace-bar button span {
          max-width: 100%;
          overflow: hidden;
          overflow-wrap: anywhere;
        }
        button[data-element-id^="workspace-tab-"] span {
          max-width: 100%;
          overflow-wrap: anywhere;
        }
        ...
        /* Self-contained styles for the provider help guides (e.g. the
           "How to get a Google Client ID?" steps). TypingMind's compiled
           Tailwind CSS only contains the utility classes the app itself
           uses, so classes like list-decimal / list-inside / space-y-* may
           not exist on the page, while Tailwind's global reset strips
           default list styling. Without these rules the guide rendered as
           an unindented wall of text with no numbers or bullets, and long
           URLs overflowed the modal on phones. */
        .cloud-sync-modal ol {
        ...
        .cloud-sync-modal #google-guide-content {
        ...
        .cloud-sync-modal #google-guide-content p {
          margin: 0 0 0.5rem 0;
          font-weight: 700;
          color: #ffffff;
        }
        .cloud-sync-modal #google-guide-content strong {
          color: #ffffff;
        }
        .cloud-sync-modal #google-guide-content .text-amber-300 {
          color: #fcd34d !important;
        }
        .cloud-sync-modal #google-guide-content a {
          color: #60a5fa !important;
          text-decoration: underline;
        }
        .cloud-sync-modal #google-guide-content code {
      ...
      // The Sync button mirrors the *live* internal structure of a native
      // workspace tab (button classes, icon <svg> classes, label <span>
      // classes) instead of hard-coded markup snapshots. This keeps it
      // pixel-aligned with the native tabs on desktop and mobile, pinned or
      // expanded, across TypingMind UI updates - and it gives other
      // extensions that use this button as their styling reference (the
      // TypingMind Tweaks extension prefers "workspace-tab-cloudsync" as its
      // template) a faithful, native-shaped structure to copy.
      const SYNC_ICON_INNER =
        ...
        "</g>";
      let lastRenderedButtonHTML = "";

      const renderLikeSettings = () => {
        // Use the Settings tab as the styling template when present; fall back
        // to the Chat tab (guaranteed to exist at this point) so the Sync
        // button still matches the workspace bar if TypingMind renames or
        // removes the settings tab in a future build.
        const templateBtn =
        ...
        // Copy classes from the template button so alignment, size, padding
        // and hover styles always match the native tabs. Filter out dynamic
        // state classes and guard against an empty class list so we never
        // wipe our own styling with an empty string.
        if (templateBtn.classList.length > 0) {
          const classesToExclude = ["active", "selected"];
          const filteredClasses = Array.from(templateBtn.classList)
            .filter((cls) => !classesToExclude.includes(cls))
            .join(" ");
          if (filteredClasses && button.className !== filteredClasses) {
            button.className = filteredClasses;
          }
        }

        // Pinned (icon-only rail) vs expanded (icon + text label): expanded
        // tabs contain a visible text <span>, pinned ones do not. Clone the
        // template's live svg/span classes instead of guessing sizes.
        const refSvg = templateBtn.querySelector("svg");
        const refSpan = Array.from(templateBtn.querySelectorAll("span")).find(
          (s) => s.textContent && s.textContent.trim().length > 0
        );
        const svgClass = (
          (refSvg && refSvg.getAttribute("class")) || "w-4 h-4 flex-shrink-0"
        ).replace(/"/g, "");
        const spanClass = refSpan
          ? (refSpan.getAttribute("class") || "").replace(/"/g, "")
          : null;
        ...
        const html =
          ...
          '<svg class="' +
          svgClass +
          '" width="18px" height="18px" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">' +
          SYNC_ICON_INNER +
          "</svg>" +
        ...
        if (html !== lastRenderedButtonHTML) {
          lastRenderedButtonHTML = html;
      ...
      // Tag the shared tab container so the crowded-bar CSS guard below can
      // reach every tab in this bar, including third-party extension tabs
      // (e.g. Tweaks) that don't use workspace-tab-* element ids.
      chatButton.parentNode.classList.add("tcs-workspace-bar");

      if (!this._tcsSyncBtnSettingsObserver) {
        const tryAttach = () => {
          // Watch whichever native tab we are mirroring; re-render our button
          // whenever its classes or children change (sidebar pin/unpin,
          // desktop <-> mobile layout switches, theme changes, etc.).
          const templateBtn =
            document.querySelector('button[data-element-id="workspace-tab-settings"]') ||
            document.querySelector('button[data-element-id="workspace-tab-chat"]');
    ...
    createModal() {
      ...
      overlay.style.cssText = `position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); display: flex; z-index: 999999; padding: 1rem; overflow-y: auto; -webkit-overflow-scrolling: touch;`;
