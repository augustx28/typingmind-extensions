/* ============================================================================
 * TypingMind - Plugin Sorter  (V12)
 * ----------------------------------------------------------------------------
 * Drag to reorder plugins in the plugin menu. Built touch-first.
 *
 * WHAT V12 CHANGES
 *  Only the section open/closed memory (MCP / Plugins / Skills collapsed with
 *  the chevron). The sorting, pinning and drag code is untouched from V11.
 *
 *  Three real bugs were in there since V5:
 *   1. One shared `busy` flag. The sweep looped over every section header, and
 *      the FIRST one that needed restoring set busy=true, which made every
 *      other header skip its restore. They were still marked as initialised,
 *      so they never got a second chance. With three sections that meant at
 *      most one ever collapsed. That is why they all came back open.
 *   2. Header title came from button.textContent only. A chevron-only button,
 *      or one whose label sits in a wrapper element, produced an empty title
 *      and was dropped entirely, so nothing was ever saved for it.
 *   3. Restore ran exactly once per DOM node, and the sweep was debounced with
 *      no floor. A busy DOM (streaming reply, our own reordering) kept pushing
 *      the timer back, and a late React re-render that reopened a section was
 *      never corrected.
 *
 *  Now: per-header programmatic flag instead of a shared one, staggered clicks
 *  so several sections restore in the same pass, a title that falls back to
 *  aria-label / wrapper text, a 1.5s heartbeat so the debounce can't starve,
 *  and up to 3 reconcile attempts per header that stop the moment the state
 *  matches. A section you touch yourself gets a 1.2s cooldown, so the script
 *  can never fight your own tap.
 *
 *  Debug: tmSorter.headers() lists every section it found, the key it stores
 *  it under, the live state and the saved state.
 *
 * THE SORTING BEHAVIOUR (unchanged from V11)
 *  No injected UI. Switch a plugin ON, it ranks to the top. Switch it OFF, it
 *  drops back into the slot it held before. Two saved lists per group:
 *    order -> your manual drag order, never touched by switching on or off
 *    pins  -> the order of the plugins currently switched on
 *  Dragging a switched-OFF row edits `order`. Dragging a switched-ON row edits
 *  `pins` only. Resting positions are seeded on first sight of a list, before
 *  anything floats, so nothing gets stranded.
 *
 *  CFG.newestOnTop  false = switched-on plugins keep your manual relative order
 *                   true  = the one you just switched on goes above the others
 *
 ...
 *  observe childList only, exactly like V8, and carry a mutation-rate watchdog
 *  that backs off instead of spinning.
 *
 * Console: tmSorter.dump() / .apply() / .float(true|false) / .debug()
 *          .headers() / .states() / .off() / .reset()
 * ========================================================================== */
(() => {
    'use strict';

    if (window.__tmSorterV12) return;
    window.__tmSorterV12 = true;

    const CFG = {
        keyOrder:  'tm_plugin_sort_v9',           // unchanged: your order lives here
        keyV8:     'tm_plugin_sort_v8',
        keyLegacy: 'tm_plugin_sort_v5_order',
        keyState:  'tm_plugin_sort_v5_toggles',   // unchanged: section states live here
        sel: {
    ...
    class OrderBook {
        constructor() {
            ...
            // pinning is on by default. Flip it once on upgrade, then respect
            // whatever tmSorter.float() was last set to.
            if (!d.pinBoot) {
                d.pinBoot = true;
                d.floatOn = true;
        ...
        fresh() {
            ...
            d.legacy = d.legacy || [];
            d.pins   = d.pins   || {};
            if (typeof d.floatOn !== 'boolean') d.floatOn = true;
            this.data = d;
            return d;
        }
        ...
        wipe() {
            this.data = { groups: {}, legacy: [], pins: {}, floatOn: true, pinBoot: true };
    ...
    // Anything that reports an expanded/collapsed state and is not a plugin row
    // control. Deliberately not "every button on the page": headers are only
    // adopted if they own a plugin list or sit beside one that does.
    ...
    // The expander that owns this list, walking up the way listKey does.
    ...
    // The other sections sitting alongside this one (the collapsed ones, whose
    // lists are not in the DOM at all, so they can't be found any other way).
    ...
    // the wrapper's own text with any plugin rows stripped out, so a
    // chevron-only header still gets a key instead of being dropped.
    ...
    class Sorter {
        ...
        /* --- styles --- */
        injectCSS() {
            if (document.getElementById('tm-sorter-v12-css')) return;
            const s = document.createElement('style');
            s.id = 'tm-sorter-v12-css';
            s.textContent = `
...
`;
        ...
        initList(list) {
            list.dataset.tmSort = '12';
            this.addHandles(list);
            this.seedResting(list);   // record true positions BEFORE anything floats
            ...
            // A tap or keypress on a row is what flips a plugin, so that is the
            // re-rank trigger. Capture phase so we still see it if TypingMind
            // stops propagation.
            const poke = (e) => {
                if (this.drag || this.dead) return;
        ...
        // Every visible row must own a slot in the resting order, otherwise a
        // plugin that gets switched off has nowhere to fall back to and ends up
        // stranded. New names are slotted next to the switched-off row they
        // rendered beside, never dumped at the end.
        seedResting(list) {
            ...
            const rows = rowsOf(list)
                .map(r => ({ name: rowName(r), on: isOn(r) }))
                .filter(x => x.name);
            if (!rows.length) return false;
            if (!rows.some(x => !have.has(x.name))) return false;

            rows.forEach((row, i) => {
                if (have.has(row.name)) return;

                let at = -1;
                for (let j = i + 1; j < rows.length; j++) {           // nearest known off row below
                    if (!rows[j].on && have.has(rows[j].name)) { at = saved.indexOf(rows[j].name); break; }
                }
                if (at < 0) {
                    for (let j = i - 1; j >= 0; j--) {                // else nearest known off row above
                        if (!rows[j].on && have.has(rows[j].name)) { at = saved.indexOf(rows[j].name) + 1; break; }
                    }
                }
                if (at < 0) at = saved.length;

                saved.splice(at, 0, row.name);
                have.add(row.name);
            });
        ...
        // Adds names that just switched on, drops names that switched off. Only
        // looks at visible rows, so a plugin hidden by search keeps its slot.
        syncPins(list) {
            const key  = listKey(list);
            const rows = rowsOf(list);
            if (!rows.length) return false;
        ...
        applyOrder(list, user) {
            ...
            // A tap on a plugin row is intent, so it skips the guard.
            const now = Date.now();
            ...
            this.seedResting(list);
            if (floatOn) this.syncPins(list);
            ...
            const want = meta.slice().sort((a, b) => {
                if (a.on !== b.on) return a.on ? -1 : 1;    // switched-on rows rank up
        ...
        // draggedName decides whether the drop edits the resting order or only
        // the pinned cluster at the top.
        saveOrder(list, draggedName) {
            ...
            if (draggedName && onSet.has(draggedName)) {
                // dragged a switched-on row: reorder the pinned cluster only, so
                // its resting slot survives being switched off later
                this.syncPins(list);
        ...
        kill() {
            ...
            const css = document.getElementById('tm-sorter-v12-css');
            if (css) css.remove();
    ...
    /* --------------------------------------- section open/closed memory (V12) */
    ...
    class StateKeeper {
        constructor(sorter) {
            ...
            // heartbeat: a busy DOM keeps resetting the debounce above, so this
            // guarantees a sweep even mid-stream
        ...
        sweep() {
            ...
            panelHeaders().forEach(h => {
                ...
                this.bind(h);
                if (this.needs(h)) {
                    setTimeout(() => this.restore(h), stagger);
                    stagger += 60;   // one section at a time, React likes it better
                }
            });
        }
        ...
        bind(h) {
            ...
            h.addEventListener('click', () => {
                if (h.__tmProg) return;      // that was us, not the user
                h.__tmUser = Date.now();
                h.__tmTries = 0;
                const grab = () => {
        ...
        restore(h) {
            ...
            h.__tmProg = true;
            try { h.click(); } catch (_) {}
            setTimeout(() => { h.__tmProg = false; }, 250);
        }
    }
    ...
    window.tmSorter = {
        ...
        off:   () => sorter.kill(),
        float: (v) => {
            if (v === undefined) return sorter.book.float();
            sorter.book.setFloat(!!v);
            document.querySelectorAll('[data-tm-sort="12"]').forEach(l => sorter.applyOrder(l, true));
            return !!v;
        },
        ...
        debug: () => Array.from(document.querySelectorAll('[data-tm-sort="12"]')).map(l => ({
            group: listKey(l),
            pinning: sorter.book.float(),
            pinned: sorter.book.pins(listKey(l)),
    ...
    console.log('[tm-sorter] v12 ready');
})();
