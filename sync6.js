...
(function () {
  ...
  const LiveSync = {
    ...
    note(key, kind, deleted = false, oldValue = null) {
      this.pending.set(key, { key, kind, deleted, oldValue });
    },
    ...
    async flush({ broadcast = true } = {}) {
      ...
      try {
        for (const [key, change] of [...this.pending]) {
          ...
          applied.push({ key, kind: change.kind, deleted: change.deleted });
        }
        if (chatListChanged) {
          ...
          if (m.type === "applied" && Array.isArray(m.changes)) {
            for (const change of m.changes) {
              if (typeof change.key === "string" && !Exclusions.test(change.key)) {
                LiveSync.note(change.key, change.kind, !!change.deleted);
              }
            }
            void LiveSync.flush({ broadcast: false });
          }
        };
      } catch {
        this.channel = null;
