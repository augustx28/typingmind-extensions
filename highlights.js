 ...
 *
 * Changes in 3.3
 *  - Highlights panel consistently uses Inter, including saved quotes.
 *  - Compact draggable color toolbar with secondary actions in a menu.
 *  - Toolbar position is remembered after dragging.
 *  - Floating launcher hides when the Highlights panel is open.
 *  - On phones, the launcher hides while TypingMind's sidebar is open.
 *  - Launcher returns automatically when the sidebar closes.
 *  - Local only. No accounts, endpoints, or external data transfers.
 *
 * Keyboard
 *  - Alt+H highlights using the last selected color.
 *  - Alt+L opens the Highlights panel.
 *  - Number keys 1 through 5 select a toolbar color.
 * ===================================================================== */
...
(() => {
    ...
    if (rank(prior.version) < rank(VERSION)) {
      console.warn(
        `[TM Highlighter] v${prior.version} is already running, so v${VERSION} did not start. ` +
          "Remove the old extension URL, then reload TypingMind."
      );
    }
    return;
  }
  ...
  const TM_NAV_SELECTOR = '[data-element-id="nav-container"]';
  ...
  function candidatesFor(record, info) {
    ...
    return findAll(source.out, target, 40).map(
      (normalizedStart) => ({
...
#tmhl-toolbar {
  ...
  isolation: isolate;
  overflow: visible;
  animation: tmhl-toolbar-enter .16s ease-out;
}
...
.tmhl-toolbar-menu.tmhl-menu-up {
  top: auto;
  bottom: calc(100% + 8px);
...
#tmhl-toast {
  ...
  text-align: center;
  pointer-events: none;
  animation: tmhl-toolbar-enter .16s ease;
}
...
`;
  ...
  function positionToolbarMenu() {
    ...
    toolbarMenu.classList.remove("tmhl-menu-up");
    toolbarMenu.style.left = "";
    toolbarMenu.style.right = "0";
    ...
    window.requestAnimationFrame(() => {
      ...
      if (!roomBelow && roomAbove) {
        toolbarMenu.classList.add("tmhl-menu-up");
      }
  ...
  function buildToolbarGrip() {
    ...
    const finish = (event) => {
      ...
      try {
        ...
        /* Ignore released pointer capture. */
      }
  ...
  function importJson(input) {
    try {
      ...
      if (!Array.isArray(items)) {
        throw new Error("No items array found.");
      }
  ...
  function buildPanel() {
    ...
    const gearTab = toolButton(
      ICON.gear,
      "Settings",
      () => setTab("settings")
    );

    const closeButton = toolButton(
      ICON.close,
      "Close",
      closePanel
    );
  ...
  function setColorFilter(color) {
    ...
    const filters =
      panel && panel._filters;

    if (!filters) return;

    Array.from(filters.children).forEach(
      (chip) => {
        const chipColor =
  ...
  function renderList() {
    ...
    records.forEach((record) => {
      listNode.appendChild(buildCard(record));
    });
  }
  ...
  function renderSettings() {
    ...
    launcherToggle.addEventListener(
      ...
      () => {
        ...
        if (settings.launcherMode === "mini") {
          toast(
            "Shrunk to a dot. Tap it to open Highlights."
          );
        }
      }
    );
    ...
    launcherField.appendChild(
      el("div", {
        class: "tmhl-help",
        text:
          "Off shrinks the launcher to a faint dot. Drag the launcher anywhere to reposition it."
      })
    );
    ...
    autoHideToggle.addEventListener(
      "click",
      () => {
        settings.autoHideMobileLauncher =
          !settings.autoHideMobileLauncher;
    ...
    autoHideField.appendChild(
      el("div", {
        class: "tmhl-help",
        text:
          "On phones and small screens, the floating Highlights button disappears while TypingMind's chat sidebar is open. It returns when the sidebar closes."
      })
    );
    ...
    resetLauncher.addEventListener(
      "click",
      () => {
        settings.launcher = {
          ...DEFAULT_SETTINGS.launcher
  ...
  /* ------------------------------------------------------------------
   * TypingMind mobile sidebar detection
   * ---------------------------------------------------------------- */

  let nativeSidebarOpenState = false;
  let sidebarOpeningHintUntil = 0;
  let sidebarSyncFrame = 0;
  let sidebarObserver = null;

  function detectNativeSidebarOpen() {
    if (!isNarrow()) return false;

    if (now() < sidebarOpeningHintUntil) {
      return true;
    }

    const visibleCloseButton = Array.from(
      document.querySelectorAll(
        TM_SIDEBAR_CLOSE_SELECTOR
      )
    ...
    const visibleOpenButton = Array.from(
      document.querySelectorAll(
        TM_SIDEBAR_OPEN_SELECTOR
      )
    ).find(isVisibleElement);
    ...
    if (
      ...
      nav.getAttribute("data-state") === "closed"
    ) {
      return false;
    }

    if (
      nav.getAttribute("data-state") === "open"
    ) {
      return true;
    }
    ...
    const substantialWidth = Math.min(
      230,
      viewport.width * 0.56
    );
  ...
  function syncNativeSidebarState() {
    nativeSidebarOpenState =
      detectNativeSidebarOpen();
  ...
  function queueNativeSidebarSync(delay) {
    window.setTimeout(() => {
      ...
      sidebarSyncFrame =
        window.requestAnimationFrame(
          syncNativeSidebarState
        );
    }, delay || 0);
  }

  function scheduleNativeSidebarChecks() {
    [0, 40, 100, 180, 320, 600, 1000].forEach(
      (delay) => {
        queueNativeSidebarSync(delay);
      }
    );
  }

  function onNativeSidebarInteraction(event) {
    if (
      !isNarrow() ||
      !(event.target instanceof Element)
    ) {
    ...
    if (opener) {
      sidebarOpeningHintUntil =
        now() + 850;

      nativeSidebarOpenState = true;
    ...
    if (closer) {
      sidebarOpeningHintUntil = 0;
      scheduleNativeSidebarChecks();
      return;
    }

    if (event.target.closest(TM_NAV_SELECTOR)) {
      scheduleNativeSidebarChecks();
    }
  }

  function startNativeSidebarWatcher() {
    document.addEventListener(
      "pointerdown",
      onNativeSidebarInteraction,
      true
    );
