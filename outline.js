// TypingMind Page Outline Extension v4.2
// Groups response headings beneath each user input.
// Toggle button or Ctrl/Cmd + Shift + O.
//
// v4.2 changes:
// - Headings inside thinking / thought-process / reasoning blocks
//   are excluded from the outline.
// - The toggle button and panel now fade in/out smoothly instead
//   of appearing and vanishing abruptly (e.g. when the mobile
//   sidebar opens or closes).

(function () {
  'use strict';

  if (window.__tmPageOutlineV42Loaded) {
    console.log('[Page Outline v4.2] Already loaded.');
    return;
  }

  window.__tmPageOutlineV42Loaded = true;

  const PANEL_ID = 'tm-page-outline-panel';
  const TOGGLE_ID = 'tm-page-outline-toggle';
  const STYLE_ID = 'tm-page-outline-styles-v42';
  ...
  function getAllModelIcons() {
    ...
    for (const avatar of avatars) {
      ...
      try {
        ...
        console.debug(
          '[Page Outline v4.2] Could not copy a model icon.',
          error
        );
      }
    }
  ...
  function init() {
    ...
    console.log(
      '[Page Outline v4.2] Loaded. Toggle with the button or Ctrl/Cmd + Shift + O.'
    );
  }
