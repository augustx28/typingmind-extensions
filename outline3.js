...
(function () {
  ...
  // ── Find model icons by their EXACT class signatures ─────────────────────────
  // Model icons have these specific patterns (from DOM inspection):
  //   1. <img class="w-full h-full object-contain" src="/assets/gemini.webp">
  //   2. <img class="flex-none flex-shrink-0 bg-white w-7 h-7 rounded-full" src="...">
  //   3. <div class="bg-[...] ... w-7 h-7 rounded-full"><svg ...></div>

  function getAllModelIcons() {
    const icons = [];

    // Pattern 1: img with object-contain + w-full + h-full (e.g. Gemini)
    document.querySelectorAll('img.object-contain').forEach((img) => {
      if (img.classList.contains('w-full') && img.classList.contains('h-full') && img.src) {
        // Exclude icons inside rendered markdown content (plugin icons live there)
        if (img.closest('.prose, .markdown-body, [class*="prose"]')) return;
        const parentBg = getComputedStyle(img.closest('div') || img).backgroundColor;
        icons.push({
          element: img,
          iconSrc: img.src,
          iconBg: (parentBg && parentBg !== 'rgba(0, 0, 0, 0)' && parentBg !== 'transparent') ? parentBg : '#ffffff'
        });
      }
    });

    // Pattern 2: img with flex-shrink-0 + w-7 + h-7 + rounded-full (e.g. GLM, DeepSeek, GPT)
    document.querySelectorAll('img.rounded-full').forEach((img) => {
      if (img.classList.contains('w-7') && img.classList.contains('h-7')
          && img.classList.contains('flex-shrink-0') && img.src) {
        if (img.closest('.prose, .markdown-body, [class*="prose"]')) return;
        const bg = getComputedStyle(img).backgroundColor;
        icons.push({
          element: img,
          iconSrc: img.src,
          iconBg: (bg && bg !== 'rgba(0, 0, 0, 0)' && bg !== 'transparent') ? bg : '#ffffff'
        });
      }
    });

    // Pattern 3: div with w-7 + h-7 + rounded-full containing an SVG (e.g. Claude)
    document.querySelectorAll('div.rounded-full').forEach((div) => {
      if (!div.classList.contains('w-7') || !div.classList.contains('h-7')) return;
      if (div.closest('.prose, .markdown-body, [class*="prose"]')) return;
      ...
      try {
        ...
        icons.push({
          element: div,
          iconSrc: dataUri,
          iconBg: (bg && bg !== 'rgba(0, 0, 0, 0)' && bg !== 'transparent') ? bg : '#c49d7f'
        });
      } catch (e) { /* skip */ }
    });
  ...
  function findModelForHeading(heading, modelIcons) {
    // Find the closest model icon that PRECEDES this heading in DOM order
    let closest = null;
    for (const mi of modelIcons) {
      const order = heading.compareDocumentPosition(mi.element);
      if (order & 2) { // DOCUMENT_POSITION_PRECEDING
        closest = mi;
      }
    }
    return closest;
