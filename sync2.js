...
@media (max-width:480px){
  ...
  .tmds-foot .tmds-btn{flex:1 1 auto}
    ...
    /**
     * Repairs the Tweaks extension's label, and nothing else.
     *
     * Tweaks styles its label by copying the class list of the first <span> in
     * a neighbouring tab. In TypingMind's markup that span is the icon
     * wrapper, so the label is laid out like an icon: it doesn't stretch to
     * the tab, doesn't centre, and doesn't take the label font size. That is
     * what makes it collide with the tab next to it.
     *
     * The values are read from a real TypingMind label at runtime, so the
     * Tweaks label ends up behaving exactly like a native one rather than
     * however I imagined a native one behaves.
     *
     * Deliberately NOT done here: shrinking type, truncating text, or clamping
     * widths on tabs this extension doesn't own. An earlier version did that
     * and it was worse than the problem.
     *
     * Tweaks reapplies its class on every DOM mutation, so this has to be a
     * stylesheet -- repairing the DOM would start a fight between two
     * observers that never ends.
     */
    applyBarCompat() {
      ...
      const rules = [
        // Lets flex items shrink instead of pushing their neighbours around.
        // Safe on any bar and changes nothing when there is room to spare.
        ".tmds-tabbar > button,.tmds-tabbar > a{min-width:0}",
      ];
      ...
      if (tweaks) {
        ...
        if (nativeLabel) {
          ...
          if (cs && cs.fontSize) {
            rules.push(
              // :not(:has(svg)) matters: a fixed Tweaks wraps its icon in a
              // span too, and applying label typography to an icon wrapper
              // would break the icon. This must only ever hit the text label.
              "#workspace-tab-tweaks > span:not(:has(svg)){" +
                `display:${cs.display};font-size:${cs.fontSize};` +
                `line-height:${cs.lineHeight};font-weight:${cs.fontWeight};` +
                `text-align:${cs.textAlign};align-self:${cs.alignSelf};` +
                `letter-spacing:${cs.letterSpacing};max-width:${cs.maxWidth};` +
      ...
      // Idempotent, so this is safe to call from a MutationObserver.
