...
const TCS_BUILD_VERSION = "2026-07-15.3";
  ...
  class CloudSyncApp {
    ...
    insertSyncButton() {
      ...
      const renderLikeSettings = () => {
        ...
        // Pinned (icon-only rail) vs expanded (icon + text label) detection:
        // expanded workspace tab buttons contain a visible text <span>, pinned
        // ones do not. This is more robust across phone/desktop layouts than
        // the legacy w-9/h-9 Tailwind class check, which silently broke
        // whenever TypingMind changed its sizing classes.
        let isPinned = false;
        if (templateBtn) {
          const hasTextLabel = Array.from(
            templateBtn.querySelectorAll("span")
          ).some((s) => s.textContent && s.textContent.trim().length > 0);
          isPinned = !hasTextLabel;
        }
        ...
        if (templateBtn && templateBtn.classList.length > 0) {
