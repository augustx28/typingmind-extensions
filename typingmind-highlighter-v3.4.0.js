...
`;
  ...
  function buildPanel() {
    ...
    panel.appendChild(
      el("div", { class: "tmhl-grab", "aria-hidden": "true" })
    );
    ...
    const listTab = toolButton(
      ICON.marker,
      "Highlights",
      () => setTab("list"),
      "tmhl-tab"
    );
    const settingsTab = toolButton(
      ICON.gear,
      "Settings",
      () => setTab("settings"),
      "tmhl-tab"
    ...
    const searchWrap = el("div", { class: "tmhl-searchwrap" });
    searchWrap.appendChild(
      el("span", {
        class: "tmhl-searchicon",
        "aria-hidden": "true",
        html: ICON.search
      })
    );
    ...
    const markSearchState = () => {
      searchWrap.setAttribute("data-filled", search.value ? "1" : "0");
    };
    ...
    const scopeChip = el("button", {
      ...
      "data-filter": "scope",
      "aria-pressed": "true",
      title: "Switch between this chat and every chat"
    });

    const scopeIcon = el("span", {
      class: "tmhl-chipicon",
      "aria-hidden": "true",
      html: ICON.chat
    });
    const scopeText = el("span", { text: "This chat" });
    scopeChip.append(scopeIcon, scopeText);

    scopeChip.addEventListener("click", () => {
      view.scope = view.scope === "chat" ? "all" : "chat";
      scopeText.textContent =
        view.scope === "chat" ? "This chat" : "All chats";
      scopeIcon.innerHTML =
        view.scope === "chat" ? ICON.chat : ICON.layers;
      renderPanel();
    });
    ...
    COLORS.forEach((color) => {
      const chip = el("button", {
        type: "button",
        class: "tmhl-chip tmhl-colorchip",
        "aria-pressed": "false",
        title: COLOR_LABEL[color],
    ...
    const footButton = (icon, label, title, action) => {
      const button = el("button", {
        type: "button",
        class: "tmhl-btn",
        title,
    ...
    const exportMarkdownButton = footButton(
      ICON.download,
      "Markdown",
    ...
    const exportJsonButton = footButton(
      ICON.archive,
      "Backup",
    ...
    const importButton = footButton(
      ICON.upload,
      "Import",
      "Import highlights from a JSON backup",
      pickImportFile
    );
    ...
    panel._tabs = { list: listTab, settings: settingsTab };
    setColorFilter("all");
  }
  ...
  function setTab(tab) {
    ...
    if (panel._tabs) {
      panel._tabs.list.setAttribute(
        "aria-pressed",
        showList ? "true" : "false"
      );
      panel._tabs.settings.setAttribute(
        "aria-pressed",
        showList ? "false" : "true"
      );
    }
  ...
  function openPanel() {
    ...
    // The selection bar sits above the panel, so clear it first.
    hideToolbar();
    scheduleTitleRefresh(true);
    scheduleCloudSync(0);
  ...
  function buildCard(record, grouped) {
    const card = el("div", {
      class: "tmhl-card",
    ...
    if (record.note && view.editingNote !== record.id) {
      ...
      note.append(
        ...
        el("span", { class: "tmhl-notetext", text: record.note })
      );
    ...
    if (!grouped) {
      const title =
        ...
        "Untitled chat";
      const chat = el("span", { class: "tmhl-chat", title });
      chat.append(
        el("span", {
          class: "tmhl-chaticon",
          "aria-hidden": "true",
        ...
        el("span", { class: "tmhl-chatname", text: title })
      );
    ...
    card.addEventListener("click", (event) => {
      if (
        ...
        event.target.closest(".tmhl-actions, .tmhl-noteedit")
      ) {
        return;
      }
  ...
  function renderList() {
    ...
    // All chats: group under a sticky header per chat.
    const groups = new Map();

    records.forEach((record) => {
      const key = record.chatId || "unknown";
      if (!groups.has(key)) groups.set(key, []);
    ...
    groups.forEach((items, key) => {
      const title =
        cleanChatTitle(items[0].chatTitle) ||
        chatTitleCache.get(key) ||
        "Untitled chat";
      const header = el("div", { class: "tmhl-group" });
      header.append(
        el("span", {
          class: "tmhl-groupicon",
          "aria-hidden": "true",
          html: ICON.chat
        }),
        el("span", { class: "tmhl-groupname", text: title, title }),
        el("b", { text: String(items.length) })
      );

      listNode.appendChild(header);
      items.forEach((record) =>
        listNode.appendChild(buildCard(record, true))
      );
    });
  }
  ...
  function enableSheetDrag() {
    ...
    const handle = panel.querySelector(".tmhl-grab");
    const head = panel.querySelector(".tmhl-head");
    const targets = [handle, head].filter(Boolean);
    if (!targets.length) return;
    ...
    const start = (event) => {
      ...
      // Never start a sheet drag from a control inside the header.
      if (
        event.target instanceof Element &&
