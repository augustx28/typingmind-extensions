  ...
  function getAllModelButtons() {
    ...
    for (const span of allSpans) {
      ...
      if (img && img.src) {
        ...
        // Claude / OpenAI use SVG with currentColor
        ...
        if (svg) {
          try {
            ...
            // Bake the actual rendered fill/stroke into every element
            // so colors survive once detached from CSS context
            ...
            if (rootCS.fill && rootCS.fill !== 'none') clone.setAttribute('fill', rootCS.fill);

            let str = new XMLSerializer().serializeToString(clone);
            str = str.replace(/currentColor/gi, rootCS.color || '#000');
            iconSrc = 'data:image/svg+xml;base64,' + btoa(unescape(encodeURIComponent(str)));
          } catch (e) { /* skip */ }
        }
      }

      modelButtons.push({ element: btn, iconSrc: iconSrc, name: span.textContent.trim() });
    }
