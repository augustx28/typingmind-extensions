 ...
 *  - Tighter highlight line, paragraph, and card spacing.
 *  - Vertical toolbar with all five colors visible on every device.
 *  - Visible color filters in the Highlights panel.
 *  - Fixed, draggable controls with no follow-selection mode.
...
(() => {
  ...
  const DEFAULT_SETTINGS = {
    ...
    toolbar: { pinned: true, xPct: 0.93, yPct: 0.35 }
  };
  ...
  function loadSettings() {
    try {
      ...
      merged.toolbar = {
        pinned: true,
        xPct: toolbarPosition.pinned && Number.isFinite(toolbarPosition.xPct)
          ? clamp(toolbarPosition.xPct, 0, 1)
          : DEFAULT_SETTINGS.toolbar.xPct,
        yPct: toolbarPosition.pinned && Number.isFinite(toolbarPosition.yPct)
          ? clamp(toolbarPosition.yPct, 0, 1)
          : DEFAULT_SETTINGS.toolbar.yPct
      };
...
/* Vertical toolbar: actions at the top, all colors, grip at the bottom */
#tmhl-toolbar {
  position: fixed;
  z-index: 2147483645;
  display: inline-flex;
...
/* Phone toolbar: a slim vertical rail with every color immediately visible */
...
.tmhl-color-rail { flex: 0 0 auto; }
#tmhl-toolbar[data-compact="true"] .tmhl-color-rail .tmhl-swatch {
  width: 40px;
  height: 40px;
}
#tmhl-toolbar[data-compact="true"] .tmhl-color-rail .tmhl-swatch::before {
  width: 21px;
  height: 13px;
}
.tmhl-toolbar-primary {
...
.tmhl-color-filters .tmhl-swatch {
  flex: 1 1 0;
  width: auto;
  min-width: 0;
  height: 38px;
...
.tmhl-color-filters .tmhl-swatch[aria-pressed="true"] {
  background: var(--tmhl-surface-solid);
}
.tmhl-color-filters .tmhl-scope-button {
  flex: 0 0 auto;
  min-height: 38px;
  padding: 0 8px;
  font-size: 11px;
...
.tmhl-quote-paragraph {
  margin: 0;
  padding: 0;
}
.tmhl-quote-paragraph + .tmhl-quote-paragraph { margin-top: .3em; }
.tmhl-note {
  margin-top: 11px;
  padding: 9px 10px;
...
`;
  ...
  function positionToolbar(rect) {
    ...
    const bounds = viewportBounds();
    toolbar.style.maxHeight = `${Math.max(44, bounds.height - 16)}px`;
    const width = toolbar.offsetWidth;
    const height = toolbar.offsetHeight;
  ...
  function buildToolbarGrip() {
    const grip = el("button", {
      ...
      title: "Drag to move. Arrow keys move the controls when focused.",
      html: ICON.grip
    });
  ...
  function buildToolbarLayout(swatches, actions, record) {
    ...
    toolbar.append(toolbarMoreButton, rail);

    if (compact && record) {
      toolbar.appendChild(uiButton(
        ICON.note,
    ...
    toolbar.appendChild(buildToolbarGrip());
  }
  ...
  function buildPanel() {
    ...
    const colorFilters = el("div", {
      class: "tmhl-color-filters",
      role: "group",
      "aria-label": "Filter by highlight color"
    });

    colorFilterButtons = ["all", ...COLORS].map(color => {
      const button = color === "all"
        ? uiButton("", "All colors", () => setColorFilter("all"), "tmhl-scope-button")
        : swatchButton(color, color === view.color, setColorFilter);

      button.dataset.color = color;
      button.setAttribute("aria-pressed", String(color === view.color));
      if (color !== "all") {
        button.setAttribute("aria-label", `Filter ${COLOR_LABEL[color].toLowerCase()} highlights`);
      }
      colorFilters.appendChild(button);
      return button;
    });

    filterRow.appendChild(scope);
    controls.append(filterRow, colorFilters);
  ...
  function syncFilterControls() {
    ...
    colorFilterButtons.forEach(button => {
      button.setAttribute("aria-pressed", String(button.dataset.color === view.color));
    });
    updatePanelCount();
  }
  ...
  function buildCard(record) {
    ...
    card.append(
      ...
      el("div", { class: "tmhl-quote" },
        record.exact.split(/\r?\n(?:[ \t]*\r?\n)+/).map(paragraph =>
          el("div", { class: "tmhl-quote-paragraph", text: paragraph })
        )
      )
    );
  ...
  function renderSettings() {
    ...
    positionField.append(
      ...
      el("p", {
        class: "tmhl-help",
        text:
          "Drag the dotted grip to move the color controls. They stay where you put them. " +
          "All five colors are always visible; tap a color to highlight or recolor text."
      })
    );
  ...
  function initialize() {
    ...
    if (!settings.seenIntro) {
      setTimeout(() => {
        toast("Select text in a reply, then choose a highlight color.");
