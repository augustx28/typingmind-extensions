(function () {
  ...
  // An older copy of this script still listed under Extensions would find the
  // button this one creates, skip creating its own, and then rewrite it with
  // its old (broken) label logic. Refuse to run twice.
  if (window.__tmUITweaks) {
    console.warn("TypingMind Tweaks: already loaded; ignoring duplicate injection.");
    return;
  }
  window.__tmUITweaks = true;

  // Shown in the modal header so you can tell at a glance which file is live.
  const TWEAKS_VERSION = "3";

  // Deliberately NOT "workspace-tab-tweaks". Older Drive Sync builds inject a
  // compatibility stylesheet keyed to that exact id, and its
  // `overflow-wrap:anywhere` removes this tab's min-content floor -- on a
  // narrow bar it then shrinks past every other tab and collides with
  // Settings. A different id makes this button invisible to that rule no
  // matter which version of the sync script is running.
  const TWEAKS_TAB_ID = "workspace-tab-uitweaks";
  ...
  function applyStylesBasedOnSettings() {
    ...
    if (teamsButton) {
      ...
      // Tailwind's space-x-* utility keys off the `hidden` attribute rather
      // than computed display, so a display:none tab would otherwise leave its
      // sibling margin behind as a phantom gap.
    ...
    if (workspaceBar) {
      workspaceBar.querySelectorAll("svg").forEach((icon) => {
        if (icon.closest("#" + TWEAKS_TAB_ID)) return;
        icon.style.color = wsIconColor ? wsIconColor : "";
      });
      const tweaksButton = document.getElementById(TWEAKS_TAB_ID);
      if (tweaksButton) {
  ...
  function createSettingsModal() {
    ...
    header.textContent = `UI Tweaks \u00B7 v${TWEAKS_VERSION}`;
  ...
  //
  // The button is a deep clone of one of TypingMind's own tabs with the icon
  // and label swapped out. Hand-assembling it meant guessing at the markup --
  // and guessing wrong: the icon lost its wrapper element, and the label
  // inherited the icon wrapper's classes rather than the label's. Cloning
  // inherits the real wrapper elements, responsive classes and type scale, so
  // the tab shrinks, wraps and hides its label on a narrow bar exactly like
  // every native tab does.
  // ---------------------------------------------------------------
  ...
  // Only ever mirror a native TypingMind tab -- mirroring another extension's
  // button copies its mistakes. An inactive tab is preferred so nested
  // elements don't arrive carrying selected-state styling.
  function getReferenceButton(workspaceBar) {
    const tabs = Array.from(
      workspaceBar.querySelectorAll('button[data-element-id^="workspace-tab-"]')
    ).filter((b) => !/tweaks|drivesync/i.test(b.dataset.elementId || ""));
  ...
  // so the label is the span holding text, not the first span in the button.
  ...
  // Swap TypingMind's selected/unselected utilities, leaving everything else.
  ...
  function buildTweaksButton(reference, active) {
    ...
    btn.id = TWEAKS_TAB_ID;
    btn.type = "button";
    btn.setAttribute("data-element-id", TWEAKS_TAB_ID);
  ...
  // Other extensions may style the workspace bar by listing TypingMind's own
  // tab ids, in which case an injected tab silently misses whatever padding or
  // sizing they apply -- and copying class names cannot recover it.
  //
  // So stop inheriting and start enforcing: read a real native tab's *computed*
  // box at runtime and pin ours to it with inline !important, which outranks
  // every stylesheet in the cascade. Re-measured on every pass and on rotation,
  // so it tracks the live layout instead of freezing a snapshot.
  const PIN_BOX_PROPS = [
    "box-sizing", "display", "flex-direction", "align-items", "justify-content",
    "gap", "row-gap", "column-gap",
    "flex-grow", "flex-shrink", "flex-basis",
    "min-width", "max-width", "min-height", "height",
    "margin-left", "margin-right", "margin-top", "margin-bottom",
    "padding-left", "padding-right", "padding-top", "padding-bottom",
    "border-radius", "position", "overflow",
  ];
  const PIN_TEXT_PROPS = [
    "display", "font-size", "line-height", "font-weight", "letter-spacing",
    "text-align", "align-self", "white-space", "overflow-wrap", "word-break",
    "min-width", "max-width",
    "margin-top", "margin-bottom", "padding-left", "padding-right",
  ];
  const PIN_ICON_PROPS = [
    "width", "height", "min-width", "min-height", "flex-shrink",
    "margin-left", "margin-right", "margin-top", "margin-bottom",
  ];

  function pinToNative(target, source, props) {
    ...
    props.forEach((p) => {
      ...
      if (!v) return;
      if (
        target.style.getPropertyValue(p) !== v ||
        target.style.getPropertyPriority(p) !== "important"
      ) {
        target.style.setProperty(p, v, "important");
      }
    });
  }

  const iconWrapper = (button) => {
    const svg = button && button.querySelector("svg");
    return svg && svg.parentElement && svg.parentElement !== button ? svg.parentElement : null;
  };

  function syncTabGeometry(btn, reference) {
    pinToNative(btn, reference, PIN_BOX_PROPS);
    pinToNative(iconWrapper(btn), iconWrapper(reference), PIN_BOX_PROPS);
    pinToNative(btn.querySelector("svg"), reference.querySelector("svg"), PIN_ICON_PROPS);
    pinToNative(tabLabelSpan(btn), tabLabelSpan(reference), PIN_TEXT_PROPS);
  }
  ...
  function syncTweaksButton() {
    ...
    // breakpoint, collapsed rail) rather than on every animation frame.
    const signature = [
    ...
    let btn = document.getElementById(TWEAKS_TAB_ID);
  ...
  function initializeTweaks() {
    ...
    // An older Drive Sync build may have left its compatibility stylesheet
    // behind. It targets a different id now, but clear it anyway.
    const staleCompat = document.getElementById("tmds-compat");
    if (staleCompat) staleCompat.remove();
    createSettingsModal();
    syncAll();
    applyGlobalUiFont();
    applyCustomFavicon();
