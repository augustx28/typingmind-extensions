...
- Workspace bar: keep tab labels inside their own buttons so a crowded bar (multiple extensions, e.g. Tweaks) does not overlap labels [2026-07-15]
...
const TCS_BUILD_VERSION = "2026-07-15.2";
    ...
    insertSyncButton() {
      ...
      if (!document.getElementById("tcs-inline-style")) {
        ...
        /* Crowded-bar guard: when several extensions add workspace tabs
           (Sync, Tweaks, ...), a tab label wider than its button bleeds into
           the neighbouring tab's label (e.g. "TweaksSettings"). Keep every
           tab's label constrained to its own button. Scoped to the label
           spans only, so icons and status dots are unaffected. */
