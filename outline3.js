...
(function () {
  ...
  // ── Find model icons by their parent button structure ────────────────────────
  // Every model response card header has this exact button structure:
  ...
  //   </button>
  // Plugin icons NEVER live inside this button pattern.

  function getAllModelIcons() {
    const icons = [];
    const buttons = document.querySelectorAll('button.rounded-lg.p-2');

    for (const btn of buttons) {
      // Must have a model name label
      const nameSpan = btn.querySelector(':scope > span.font-semibold');
      if (!nameSpan) continue;

      // Must have the icon wrapper span
      const iconSpan = btn.querySelector(':scope > span.relative');
      if (!iconSpan) continue;

      // Extract icon: try img first (Gemini, GLM, DeepSeek)
      const img = iconSpan.querySelector('img');
      if (img && img.src) {
        const parentDiv = img.closest('div.rounded-full');
        let bg = '#ffffff';
        if (parentDiv) {
          const c = getComputedStyle(parentDiv).backgroundColor;
          if (c && c !== 'rgba(0, 0, 0, 0)' && c !== 'transparent') bg = c;
        } else {
          const c = getComputedStyle(img).backgroundColor;
          if (c && c !== 'rgba(0, 0, 0, 0)' && c !== 'transparent') bg = c;
        }
        icons.push({ element: btn, iconSrc: img.src, iconBg: bg });
        continue;
      }

      // Try SVG icon (Claude, GPT)
      const svgDiv = iconSpan.querySelector('div.rounded-full');
      if (svgDiv) {
        const svg = svgDiv.querySelector('svg');
        if (svg) {
          let bg = '#666';
          const c = getComputedStyle(svgDiv).backgroundColor;
          if (c && c !== 'rgba(0, 0, 0, 0)' && c !== 'transparent') bg = c;
          try {
            ...
            icons.push({ element: btn, iconSrc: dataUri, iconBg: bg });
          } catch (e) { /* skip */ }
        }
      }
    }
