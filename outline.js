...
function injectStyles() {
    ...
    style.textContent = `
        ...
        #${TOGGLE_ID}.chat-visible { display: flex; }
        #${TOGGLE_ID}:hover { background: rgba(180, 180, 180, 0.35); color: rgba(60, 60, 60, 0.9); }
        #${TOGGLE_ID}:active { background: rgba(180, 180, 180, 0.45); }
        #${TOGGLE_ID}.active { background: rgba(200, 155, 60, 0.2); color: rgba(190, 140, 50, 0.9); border-color: rgba(200, 155, 60, 0.3); }
        #${TOGGLE_ID}.active:hover { background: rgba(200, 155, 60, 0.3); }
        ...
        .dark #${TOGGLE_ID}:hover { background: rgba(255, 255, 255, 0.15); color: rgba(220, 220, 220, 0.8); }
        .dark #${TOGGLE_ID}:active { background: rgba(255, 255, 255, 0.2); }
        .dark #${TOGGLE_ID}.active { background: rgba(210, 165, 70, 0.18); color: rgba(220, 175, 80, 0.9); border-color: rgba(210, 165, 70, 0.22); }
        .dark #${TOGGLE_ID}.active:hover { background: rgba(210, 165, 70, 0.28); }
        ...
        .dark #${PANEL_ID} .outline-header { color: #666; border-bottom-color: rgba(255, 255, 255, 0.06); }
        #${PANEL_ID} .outline-header .outline-count { font-weight: 400; font-size: 9px; color: #bbb; }
        .dark #${PANEL_ID} .outline-header .outline-count { color: #555; }

        /* ── Outline List ───────────────────────────────────────────────── */
        #${PANEL_ID} .outline-list { list-style: none; margin: 0; padding: 4px 0; }
        ...
        #${PANEL_ID} .outline-item:hover { background: rgba(0, 0, 0, 0.03); border-left-color: rgba(100, 100, 100, 0.4); }
        .dark #${PANEL_ID} .outline-item:hover { background: rgba(255, 255, 255, 0.04); border-left-color: rgba(200, 200, 200, 0.3); }
        #${PANEL_ID} .outline-item[data-level="1"] { font-weight: 600; font-size: 12px; padding-left: 12px; }
        #${PANEL_ID} .outline-item[data-level="2"] { font-weight: 500; font-size: 11.5px; padding-left: 22px; }
        #${PANEL_ID} .outline-item[data-level="3"] { font-weight: 400; font-size: 11px; padding-left: 32px; color: #666; }
        .dark #${PANEL_ID} .outline-item[data-level="3"] { color: #888; }
        #${PANEL_ID} .outline-item[data-level="4"] { font-weight: 400; font-size: 10.5px; padding-left: 42px; color: #888; }
        .dark #${PANEL_ID} .outline-item[data-level="4"] { color: #666; }
        #${PANEL_ID} .outline-empty { padding: 16px 12px; text-align: center; color: #aaa; font-size: 11px; }
        .dark #${PANEL_ID} .outline-empty { color: #555; }

        /* ── Scrollbar ──────────────────────────────────────────────────── */
        #${PANEL_ID}::-webkit-scrollbar { width: 3px; }
        #${PANEL_ID}::-webkit-scrollbar-track { background: transparent; }
        #${PANEL_ID}::-webkit-scrollbar-thumb { background: rgba(128, 128, 128, 0.2); border-radius: 3px; }
    `;
...
function getHeadings() {
    ...
    const results = [];
    let lastMessageBlock = null;

    headings.forEach((h, i) => {
        const text = h.textContent.trim();
        if (!text) return;
        const level = parseInt(h.tagName.charAt(1), 10);
        if (!h.id) { h.id = `tm-outline-heading-${i}-${Date.now()}`; }

        let iconSrc = null;
        const messageBlock = h.closest('[data-element-id*="message"], [class*="message"], .group');
        
        if (messageBlock && messageBlock !== lastMessageBlock) {
            lastMessageBlock = messageBlock;
            const avatarImg = messageBlock.querySelector('img');
            if (avatarImg && avatarImg.src) {
                iconSrc = avatarImg.src;
            }
        }

        results.push({ text, level, id: h.id, element: h, iconSrc });
    });
    return results;
}
