...
(function () {
  ...
  function findMessageBlock(headingEl) {
    // Walk up from the heading to find the message-level container.
    // Then grab the FIRST small img in that container (the model avatar).
    // The avatar always appears before content images in DOM order.
    let node = headingEl;
    for (let i = 0; i < 25 && node && node !== document.body; i++) {
      ...
      // Collect all imgs in this container
      ...
      for (const img of allImgs) {
        ...
        // Model avatars are small (typically under 50px) and appear
        // near the top of the message. Skip large content images.
        if (w > 0 && w <= 64 && h > 0 && h <= 64) {
          // Verify this img is NOT inside the rendered markdown content area
          // by checking it appears before the heading in document order
