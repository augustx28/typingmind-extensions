...
(function () {
  ...
  // ── Find the model icon for the active response ─────────────────────────────
  function findMessageBlock(headingEl) {
    // Walk up from the heading to find the message container that has
    // the model tab bar: buttons with data-tooltip-id="global".
    // The active model tab has "border-blue-500" in its class list.
    let node = headingEl;
    for (let i = 0; i < 30 && node && node !== document.body; i++) {
      node = node.parentElement;
      if (!node) break;

      // Look for the model tab bar buttons
      const modelBtns = node.querySelectorAll('button[data-tooltip-id="global"][data-tooltip-content]');
      if (modelBtns.length === 0) continue;

      // Find the active model tab (border-blue-500)
      let activeBtn = null;
      ...
      // Fallback to first model button if none is active
      if (!activeBtn) activeBtn = modelBtns[0];

      // Try to get an img from the active button
      const img = activeBtn.querySelector('img');
      if (img && img.src) {
        return { container: node, iconSrc: img.src, modelName: activeBtn.getAttribute('data-tooltip-content') };
      }

      // Some models (e.g. Claude) use an inline SVG instead of img.
      // Create a tiny data URI from the SVG if present.
      ...
      if (svg) {
        try {
          ...
          return { container: node, iconSrc: dataUri, modelName: activeBtn.getAttribute('data-tooltip-content') };
        } catch (e) {
          // SVG serialization failed, skip icon
        }
      }

      // Model found but no extractable icon
      return { container: node, iconSrc: null, modelName: activeBtn.getAttribute('data-tooltip-content') };
    }
    return { container: null, iconSrc: null, modelName: null };
  }
