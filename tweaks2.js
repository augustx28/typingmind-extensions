...
(function () {
  ...
  function syncTweaksButton() {
    ...
    if (!tweaksButton) {
      ...
      // FIX: wrap the icon, the way TypingMind does.
      //
      // This used to append the <svg> straight into the button, while every
      // native tab wraps its icon in a <span>. A different child structure
      // means a different width in the flex row, which is the other half of
      // the label collision. Same structure in, same size out.
      ...
      tweaksButton.append(iconWrapper, textSpan);
    ...
    if (mine.iconWrap) {
      if (ref.iconWrap) {
        const refWrapClass = ref.iconWrap.getAttribute("class") || "";
        if (mine.iconWrap.getAttribute("class") !== refWrapClass) {
          mine.iconWrap.setAttribute("class", refWrapClass);
        }
        mine.iconWrap.style.display = "";
      } else {
        // TypingMind isn't wrapping its icons here. Rather than leave an extra
        // box in the flex row, make our wrapper transparent to layout so the
        // icon behaves as a direct child, exactly like the reference.
        if (mine.iconWrap.getAttribute("class")) mine.iconWrap.setAttribute("class", "");
        mine.iconWrap.style.display = "contents";
      }
    }
    ...
    if (mine.svg) {
      ...
      mine.svg.style.color = getSetting(settingsKeys.workspaceIconColor, defaultWorkspaceIconColorVisual);
    }
    ...
    if (mine.label) {
      if (ref.label) {
        ...
        // Follow the neighbouring label's visibility, so a collapsed icon-only
        // rail hides ours too.
        mine.label.style.display =
          window.getComputedStyle(ref.label).display === "none" ? "none" : "";
      } else {
        // Reference tab shows no label at all (icon-only rail): hide ours.
        mine.label.style.display = "none";
      }
    }
