...
(function () {
  ...
  // ── Find model icons by the universal w-7 h-7 rounded-full pattern ──────────
  // Every model avatar in TypingMind uses w-7 h-7 rounded-full:
  //   Gemini: <div class="... w-7 h-7 rounded-full"><img class="object-contain" ...></div>
  //   Claude: <div class="bg-[#c49d7f] ... w-7 h-7 rounded-full"><svg ...></div>
  //   GLM:    <img class="... w-7 h-7 rounded-full" src="...">
  // Plugin icons do NOT use this class pattern.
  ...
  function getAllModelIcons() {
    ...
    for (const avatar of avatars) {
      ...
      // Case 3: avatar is a <div> containing an <svg> (Claude, GPT)
      const svg = avatar.querySelector('svg');
      if (svg) {
        try {
          // Resolve currentColor: get the computed text color from the avatar
          // GPT uses text-white on black bg, so currentColor = white
          const computedColor = getComputedStyle(avatar).color || '#000000';
          const clone = svg.cloneNode(true);
          clone.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
          let str = new XMLSerializer().serializeToString(clone);
          // Replace all instances of currentColor with the resolved color
