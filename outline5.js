...
(function () {
  ...
  function getAllModelButtons() {
    // Find all spans with font-semibold + text-xs (model name labels)
    const allSpans = document.querySelectorAll('span.font-semibold.text-xs');
    const modelButtons = [];
    ...
    for (const span of allSpans) {
      ...
      // Must have rounded-lg and p-2 (response card model button pattern)
      if (!btn.classList.contains('rounded-lg') || !btn.classList.contains('p-2')) continue;
      ...
      if (img && img.src) {
        iconSrc = img.src;
        // Grab background from the img or its parent wrapper div
        const imgParent = img.parentElement;
        const parentBg = imgParent ? getComputedStyle(imgParent).backgroundColor : null;
        const imgBg = getComputedStyle(img).backgroundColor;
        // Use parent bg first (wrapper div), fallback to img bg
        if (parentBg && parentBg !== 'rgba(0, 0, 0, 0)' && parentBg !== 'transparent') {
          iconBg = parentBg;
        } else if (imgBg && imgBg !== 'rgba(0, 0, 0, 0)' && imgBg !== 'transparent') {
          iconBg = imgBg;
        }
      } else {
        // Claude uses SVG
        ...
        if (svg) {
          // Grab background from SVG's parent wrapper
          const svgParent = svg.parentElement;
          if (svgParent) {
            const bg = getComputedStyle(svgParent).backgroundColor;
      ...
      modelButtons.push({ element: btn, iconSrc: iconSrc, iconBg: iconBg, name: span.textContent.trim() });
    }
