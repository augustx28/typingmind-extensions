(function () {
  ...
  // Guesses family name, weight and style from a font file name,
  // e.g. "TiemposText-BoldItalic.otf" -> { family: "TiemposText", weight: 700, italic: true }
  function parseFontFileName(fileName) {
    ...
    tokens.forEach((token) => {
      ...
      if (work && /^\d{3}$/.test(work) && +work >= 100 && +work <= 950) {
        weight = +work;
        work = "";
      }
      if (work) familyTokens.push(work);
    });
  ...
  function getChatFontStore() {
    ...
    if (Array.isArray(raw.files)) {
      return { activeFamily: raw.activeFamily || null, files: raw.files.filter((f) => f && f.dataUrl) };
    }
    if (raw.dataUrl) {
