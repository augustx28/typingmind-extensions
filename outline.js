...
(function () {
  ...
  function findMessageBlock(headingEl) {
    // Walk up from the heading to find the response card container.
    // The card contains the model name + icon at its top, then the content.
    // We need the card boundary that does NOT include the shared model tab bar.

    // Step 1: Find the response card. Walk up and find the deepest container
    // that holds the heading but does NOT contain tab bar buttons.
    let card = null;
    let node = headingEl;
    for (let i = 0; i < 30 && node && node !== document.body; i++) {
      ...
      // If this container has tab bar buttons, we've gone too far up.
      // The card must be BELOW the tab bar in the DOM tree.
      const hasTabBar = node.querySelector('button[data-tooltip-id="global"][data-tooltip-content]');
      if (hasTabBar) {
        // Use the previous valid card candidate
        break;
      }
      card = node;
    }

    if (!card) return { container: null, iconSrc: null };

    // Step 2: Grab the FIRST small img in this card. It's always the model
    // avatar in the card header, not a plugin/content image.
    ...
    for (const img of allImgs) {
      ...
      if (w > 0 && w <= 48 && h > 0 && h <= 48) {
        return { container: card, iconSrc: src };
      }
    }

    // Step 3: Check for inline SVG model icons (e.g. Claude)
    ...
    for (const svg of allSvgs) {
      ...
      if (sw > 0 && sw <= 36 && sh > 0 && sh <= 36) {
        // Make sure it's near the top of the card, not a random content SVG
        const svgTop = svg.getBoundingClientRect().top;
        const cardTop = card.getBoundingClientRect().top;
        if (svgTop - cardTop > 80) continue; // too far down, skip
        try {
          const clone = svg.cloneNode(true);
          clone.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
          const str = new XMLSerializer().serializeToString(clone);
          const dataUri = 'data:image/svg+xml;base64,' + btoa(unescape(encodeURIComponent(str)));
          return { container: card, iconSrc: dataUri };
        } catch (e) { /* skip */ }
      }
    }

    return { container: card, iconSrc: null };
  }
