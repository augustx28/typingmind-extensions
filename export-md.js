/* ==========================================================================
 * TypingMind Chat Export (.md)  v1.2
 * --------------------------------------------------------------------------
 ...
 *   as the model wrote them. Each answer starts with "### Agent" (left out
 *   when the chat has no agent) and "### Model" on its own line.
...
(function () {
  ...
  // Each answer gets its own label: "### Agent" on one line (left out when the
  // chat has no agent) and "### Model" on the next.
  function labelFor(modelId, ctx) {
    const model = cleanLabel(modelLabel(modelId, ctx.chat));
    const lines = [];
    if (ctx.agent) lines.push('### ' + ctx.agent);
