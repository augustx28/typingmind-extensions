...
(function () {
  ...
  function getLikelyContentRoot(headingEl) {
    let node = headingEl;
    while (node && node !== document.body) {
      const meta = getElementMeta(node);
      if (
        /\bmarkdown\b|\bprose\b|\bmessage-content\b|\bchat-content\b|\bresponse-content\b|\bassistant-message-content\b/.test(meta)
      ) {
        return node;
      }
  ...
  function scoreAvatarCandidate(img, headingEl, contentRoot) {
    if (!(img instanceof HTMLImageElement)) return -Infinity;
    if (!img.src && !img.currentSrc) return -Infinity;
    if (headingEl.contains(img)) return -Infinity;
    if (contentRoot && contentRoot.contains(img)) return -Infinity;

    const imgRect = img.getBoundingClientRect();
    const headingRect = headingEl.getBoundingClientRect();

    if (!imgRect.width || !imgRect.height) return -Infinity;

    const meta = getElementMeta(img);

    let score = 0;

    // Hard rejects / strong penalties
    if (/plugin|extension|tool|attachment|thumbnail|thumb|preview|file|upload|image-block|gallery/.test(meta)) score -= 120;
    if (/user-avatar|user profile/.test(meta)) score -= 120;
    if (/\buser\b|\byou\b|\bprofile\b/.test(meta)) score -= 80;

    // Positive hints
    if (/\bmodel\b|\bassistant\b|\bgpt\b|\bclaude\b|\bgemini\b|\bgrok\b|\bdeepseek\b|\bllama\b|\bopenai\b|\banthropic\b/.test(meta)) score += 60;
    if (/\bavatar\b|\bicon\b/.test(meta)) score += 20;

    // Geometry: avatar should usually sit to the LEFT of the heading
    if (imgRect.right <= headingRect.left + 20) {
      score += 50;
    } else {
      score -= 60;
    }

    // Prefer images near the heading vertically
    const imgCenterY = imgRect.top + (imgRect.height / 2);
    const headingTop = headingRect.top;
    const verticalDelta = Math.abs(imgCenterY - headingTop);
    score += Math.max(0, 70 - verticalDelta);

    // Prefer small avatar-like sizes
    if (imgRect.width >= 14 && imgRect.width <= 40 && imgRect.height >= 14 && imgRect.height <= 40) {
      score += 20;
    }
    if (imgRect.width > 56 || imgRect.height > 56) {
      score -= 20;
    }

    // Prefer close horizontal gap
    const horizontalGap = Math.abs(headingRect.left - imgRect.right);
    score += Math.max(0, 30 - horizontalGap);

    // Slight preference for being above or aligned rather than below
    if (imgRect.top <= headingRect.top + 20) {
      score += 10;
    }
  ...
  function getHeadingModelIconSrc(headingEl) {
    const chatContainer = getChatContainer();
    const contentRoot = getLikelyContentRoot(headingEl);
    ...
    for (let depth = 0; node && depth < 10; depth++, node = node.parentElement) {
      scopes.push(node);
      if (node.parentElement) scopes.push(node.parentElement);
      if (node.previousElementSibling) scopes.push(node.previousElementSibling);
      if (node.parentElement && node.parentElement.previousElementSibling) {
        scopes.push(node.parentElement.previousElementSibling);
      }
      if (node === chatContainer) break;
    }

    const imgs = uniqueElements(
    ...
    for (const img of imgs) {
      const score = scoreAvatarCandidate(img, headingEl, contentRoot);
      if (score > bestScore) {
        bestScore = score;
        bestImg = img;
      }
    ...
    if (!bestImg || bestScore < 30) return '';
  ...
  function refreshOutline() {
    ...
    headings.forEach((h) => {
      ...
      const displayLevel = Math.min(h.level - minLevel + 1, 4);
      li.setAttribute('data-level', displayLevel);
      li.title = h.text;

      if (displayLevel === 1) {
        const iconSrc = getHeadingModelIconSrc(h.element);

        if (iconSrc) {
          const wrap = document.createElement('div');
          wrap.className = 'outline-item-content';

          const img = document.createElement('img');
          img.className = 'outline-model-icon';
          img.src = iconSrc;
          img.alt = '';
          img.loading = 'lazy';
          img.decoding = 'async';
