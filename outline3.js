...
(function () {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      /* ── Model Icon in Outline ──────────────────────────────────────── */
      #${PANEL_ID} .outline-icon {
        width: 14px;
        height: 14px;
        min-width: 14px;
        border-radius: 3px;
  ...
  // ── Find model icon for a heading element ──────────────────────────────────
  function findModelIcon(headingEl) {
    // Strategy: walk up from the heading to find the message-level container,
    // then locate the model avatar img that lives outside the prose/content area.
    const proseParent = headingEl.closest('[class*="prose"], [class*="markdown"], [class*="message-content"]');
    let messageBlock = proseParent ? proseParent.parentElement : null;

    // Walk up a few levels to find a reasonable message wrapper
    if (!messageBlock) {
      messageBlock = headingEl.parentElement;
    }
    let depth = 0;
    while (messageBlock && messageBlock !== document.body && depth < 12) {
      // Look for an img that is NOT inside the prose/content area
      const imgs = messageBlock.querySelectorAll('img[src]');
      for (const img of imgs) {
        // Skip images that are inside the content/prose block (inline images in the response)
        if (proseParent && proseParent.contains(img)) continue;

        const src = img.getAttribute('src') || '';
        // Skip tiny tracking pixels, data URIs, and SVG placeholders
        if (!src || src.startsWith('data:image/svg') || src.length < 10) continue;

        // Avatar images are typically small; check natural or rendered size
        const w = img.naturalWidth || img.width || img.offsetWidth || 0;
        const h = img.naturalHeight || img.height || img.offsetHeight || 0;
        // Accept if it looks like an icon/avatar (roughly square, small-ish)
        // or if we can't measure it yet (lazy loaded), accept any non-content img
        if ((w > 0 && w <= 80 && h <= 80) || w === 0) {
          return src;
        }
      }
  ...
  function refreshOutline() {
    ...
    headings.forEach((h) => {
      ...
      // Prepend model icon if available
      if (h.iconSrc) {
      ...
      // Heading text in a span for proper ellipsis with flexbox
