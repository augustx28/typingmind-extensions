...
(function () {
  ...
  // ── Find the response container for a heading ──────────────────────────────
  function findResponseContainer(headingEl) {
    // Walk up to find a container that holds the model selector button
    // and the response content. This is typically a large div wrapping
    // the model tab bar + rendered markdown.
    let node = headingEl;
    for (let i = 0; i < 25 && node && node !== document.body; i++) {
      node = node.parentElement;
      if (!node) break;
      // Check if this container has a model icon button
      const modelBtn = node.querySelector('button[data-tooltip-id="global"] img[src*="/assets/"]');
      if (modelBtn) return node;
    }
    return null;
  }

  // ── Find model icon from response container ────────────────────────────────
  function findModelIcon(container) {
    if (!container) return null;
    // Target the specific model selector button that TypingMind renders
    // Structure: button[data-tooltip-id="global"] > span > div > img[src*="/assets/"]
    const img = container.querySelector('button[data-tooltip-id="global"] img[src*="/assets/"]');
    if (img && img.src) return img.src;
    // Fallback: any img with /assets/ path inside a button with tooltip
    const btn = container.querySelector('button[data-tooltip-content] img');
    if (btn && btn.src && btn.src.includes('/assets/')) return btn.src;
    return null;
  }
  ...
  function getHeadings() {
    ...
    headings.forEach((h, i) => {
      ...
      let iconSrc = null;
      const respContainer = findResponseContainer(h);
      if (respContainer && !seenContainers.has(respContainer)) {
        seenContainers.add(respContainer);
        iconSrc = findModelIcon(respContainer);
      }
