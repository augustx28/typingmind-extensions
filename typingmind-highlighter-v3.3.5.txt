 ...
 *  - Adds optional sync through a private GitHub repository.
 *
...
(() => {
  ...
  async function runCloudSync(config, generation) {
    ...
    check();
    if (repo.private !== true) throw cloudError("Use a private repository for your highlight data. Nothing was uploaded.", 400);
    if (repo.archived) throw cloudError("This repository is archived. Choose an active private repository.", 400);
    const branch = repo.default_branch;
    if (!branch) throw cloudError("Initialize the private repository with a README first.", 400);
  ...
  function syncCloud(manual) {
    ...
    syncStatus("Syncing...", "Syncing highlights with your private repository...");
    cloudPromise = (async () => {
      try {
  ...
  function appendSyncSettings() {
    ...
    const repo = el("input", {
      class: "tmhl-search", type: "text", placeholder: "GitHub owner/private-repository",
      "aria-label": "GitHub private repository", autocomplete: "off", autocapitalize: "none", spellcheck: "false"
    });
    ...
    field.appendChild(el("div", { class: "tmhl-help", text:
      "Use a separate private repository with a README. Allow this repository in your token and give it Contents: Read and write. Enter the same repository on every device. Your token is saved in this browser and can be read by scripts running on this TypingMind page. Never put it in your published JS."
    }));
...
`;
    ...
    style.textContent += `
      #tmhl-panel .tmhl-meta { flex-wrap: wrap; }
      #tmhl-panel .tmhl-meta .tmhl-chat { flex: 1 1 80px; min-width: 0; }
      #tmhl-panel .tmhl-actions { flex: 0 0 auto; }
      @media (max-width: 820px) {
        #tmhl-panel .tmhl-meta > span:nth-child(2) { display: none; }
        #tmhl-panel .tmhl-meta .tmhl-chat {
          order: -1; flex: 0 0 100%; max-width: 100%;
  ...
  function buildCard(record) {
...

    {
      ...
      meta.appendChild(
        el("span", { class: "tmhl-chat", text: cleanChatTitle(record.chatTitle) || chatTitleCache.get(record.chatId) || "Untitled chat", title: cleanChatTitle(record.chatTitle) || chatTitleCache.get(record.chatId) || "Untitled chat" })
      );
    }
