...
(function () {
  ...
  // ── Find model icons ────────────────────────────────────────────────────────
  // The response card model button has this exact structure:
  //   <button class="... rounded-lg p-2 ...">
  //     <span class="relative"><img .../></span>
  //     <span class="text-gray-500 font-semibold text-xs">Model Name</span>
  //   </button>
  // Strategy: collect ALL model buttons on the page, then for each heading
  // find the closest one that precedes it in document order.

  function getAllModelButtons() {
    // Model card buttons have this EXACT pattern (from user-provided HTML):
    //   <button class="flex items-center gap-2 hover:bg-gray-100 rounded-lg p-2
    //     transition-colors active:bg-gray-200 dark:hover:bg-white/10">
    //     <span class="relative"> icon here </span>
    //     <span class="text-gray-500 font-semibold text-xs">Model Name</span>
    //   </button>
    //
    // The triple class "text-gray-500 font-semibold text-xs" on the name span
    // is the unique fingerprint. Plugin icons never have this.
    ...
    for (const btn of allBtns) {
      ...
      if (btn.getAttribute('data-tooltip-id') === 'global') continue;

      // Look for the name span with ALL THREE classes
      const nameSpans = btn.querySelectorAll('span');
      let nameSpan = null;
      for (const s of nameSpans) {
        if (s.classList.contains('text-gray-500')
      ...
      if (img && img.src) {
        iconSrc = img.src;
        // Grab background from the img's wrapper div or itself
        const imgParent = img.parentElement;
        if (imgParent) {
          const parentBg = getComputedStyle(imgParent).backgroundColor;
          if (parentBg && parentBg !== 'rgba(0, 0, 0, 0)' && parentBg !== 'transparent') {
            iconBg = parentBg;
          }
        }
        if (!iconBg) {
          const imgBg = getComputedStyle(img).backgroundColor;
          if (imgBg && imgBg !== 'rgba(0, 0, 0, 0)' && imgBg !== 'transparent') {
            iconBg = imgBg;
          }
        }
      } else {
        // Claude uses SVG
        const svgWrapper = btn.querySelector('div[class*="bg-"]');
        const svg = btn.querySelector('svg');
        if (svgWrapper) {
          const bg = getComputedStyle(svgWrapper).backgroundColor;
        ...
        if (svg) {
          try {
            ...
            iconSrc = 'data:image/svg+xml;base64,' + btoa(unescape(encodeURIComponent(str)));
          } catch (e) { /* skip */ }
        }
      }
  ...
  function findModelForHeading(heading, modelButtons) {
    // Find the closest model button that PRECEDES this heading in DOM order
    let closest = null;
    for (const mb of modelButtons) {
      const order = heading.compareDocumentPosition(mb.element);
  ...
  function getHeadings() {
    ...
    const modelButtons = getAllModelButtons();
    const results = [];
    ...
    headings.forEach((h, i) => {
      ...
      // Only attach model icon to the first heading per model button
      let iconSrc = null;
      let iconBg = null;
      const model = findModelForHeading(h, modelButtons);
      ...
      results.push({ text, level, id: h.id, element: h, iconSrc, iconBg });
    });
    return results;
  }
  ...
  function refreshOutline() {
    ...
    headings.forEach((h) => {
      ...
      if (h.iconSrc) {
        ...
        icon.alt = '';
        if (h.iconBg) {
          icon.style.backgroundColor = h.iconBg;
        } else {
