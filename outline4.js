...
(function () {
  ...
  if (window.__tmPageOutlineV4Loaded) {
    console.log('[Page Outline v4] Already loaded.');
    return;
  }
  window.__tmPageOutlineV4Loaded = true;
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      #${PANEL_ID} .outline-prompt {
        ...
        line-height: 1.35;
        color: #554318;
        background: rgba(200, 155, 60, 0.085);
        border-top: 1px solid rgba(200, 155, 60, 0.13);
        border-left-color: rgba(200, 155, 60, 0.5);
      }
      ...
      #${PANEL_ID} .outline-prompt:hover {
        ...
        border-left-color: rgba(190, 140, 40, 0.85);
      }

      .dark #${PANEL_ID} .outline-prompt {
        color: #d7bd79;
        background: rgba(210, 165, 70, 0.08);
        border-top-color: rgba(210, 165, 70, 0.11);
        border-left-color: rgba(210, 165, 70, 0.45);
      }

      .dark #${PANEL_ID} .outline-prompt:hover {
        background: rgba(210, 165, 70, 0.14);
        border-left-color: rgba(220, 175, 80, 0.8);
      }

      #${PANEL_ID} .outline-prompt-badge {
        flex: 0 0 auto;
        min-width: 24px;
        padding: 1px 4px;
        border-radius: 4px;
        ...
        color: rgba(150, 105, 20, 0.95);
        background: rgba(200, 155, 60, 0.16);
      }

      .dark #${PANEL_ID} .outline-prompt-badge {
        color: rgba(225, 185, 95, 0.95);
        background: rgba(210, 165, 70, 0.15);
      }
  ...
  function getAllModelIcons() {
    ...
    for (const avatar of avatars) {
      ...
      if (avatar.closest(`#${PANEL_ID}, #${TOGGLE_ID}`)) continue;
      if (avatar.closest(USER_MESSAGE_SELECTOR)) continue;
      ...
      try {
        ...
        console.debug(
          '[Page Outline v4] Could not copy a model icon.',
          error
        );
      }
    }
  ...
  function getOutlineData() {
    ...
    if (aiResponses.length > 0) {
      headings = headings.filter((heading) => {
        return Boolean(heading.closest(AI_RESPONSE_SELECTOR));
      });
    } else {
      headings = headings.filter((heading) => {
    ...
    for (const item of rawItems) {
      if (item.type === 'prompt') {
        ...
        const fallbackText = `Input ${questionNumber}`;
  ...
  function createPromptItem(entry) {
    ...
    badge.className = 'outline-prompt-badge';
    badge.textContent = `Q${entry.questionNumber}`;
  ...
  function refreshOutline() {
    ...
    const countElement = panel.querySelector('.outline-count');
    ...
    if (questionCount) {
      countParts.push(`${questionCount}Q`);
    }

    if (headingCount) {
      countParts.push(`${headingCount}H`);
    }
    ...
    if (entries.length === 0) {
      ...
      for (const entry of entries) {
        if (entry.type === 'prompt') {
          fragment.appendChild(createPromptItem(entry));
        } else {
          fragment.appendChild(createHeadingItem(entry));
        }
      }
    }
  ...
  function init() {
    ...
    console.log(
      '[Page Outline v4] Loaded. Toggle with the button or Ctrl/Cmd + Shift + O.'
    );
  }
