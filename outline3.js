...
(function () {
  ...
  // ── Added: find nearest model icon for a heading ───────────────────────────
  function scoreImage(img) {
    ...
    // Strongly avoid user avatars
    if (/user-avatar|user profile/.test(meta)) score -= 100;
    if (/\buser\b|\byou\b|\bprofile\b/.test(meta)) score -= 25;

    // Prefer likely model/assistant icons
    if (/\bmodel\b|\bassistant\b|\bgpt\b|\bclaude\b|\bgemini\b|\bgrok\b|\bdeepseek\b|\bllama\b|\bopenai\b|\banthropic\b/.test(meta)) score += 40;
    if (/\bavatar\b|\bicon\b/.test(meta)) score += 10;

    // Prefer small icon-like images
    const rect = img.getBoundingClientRect();
    const w = rect.width || img.width || 0;
    const h = rect.height || img.height || 0;
    if (w > 0 && h > 0 && w <= 40 && h <= 40) score += 6;
    if (w > 60 || h > 60) score -= 8;

    if (img.currentSrc || img.src) score += 1;

    return score;
  }

  function getBestImageSrc(root) {
  ...
  function getHeadingModelIconSrc(headingEl) {
    ...
    for (let depth = 0; node && depth < 10; depth++, node = node.parentElement) {
      const regions = [
        ...
        node.parentElement,
        node.parentElement && node.parentElement.firstElementChild
      ];

      for (const region of regions) {
        if (!region || checked.has(region)) continue;
        checked.add(region);

        const src = getBestImageSrc(region);
        if (src) return src;
      }
    }
  ...
  function refreshOutline() {
    ...
    headings.forEach((h) => {
      ...
      // Add model icon only to the beginning/top-level part of the outline
      if (displayLevel === 1) {
