/* ==========================================================================
 * TypingMind Chat Export (.md)  v1.3.6
 * --------------------------------------------------------------------------
 ...
 *   as the model wrote them. Each question has an H5 "Input" heading and a
 *   collapsed grey quote box; each answer
 ...
 *
 * v1.3.6: restore the approved answer-heading formatting from v1.3.4;
 *         model labels stay H4 and the collapsed Input boxes stay H5.
 * v1.3.5: tried H5/H6 answer headings; reverted in v1.3.6.
 * v1.3.4: rename the input heading and collapsed box label to "Input".
 * v1.3.3: H5 "You" headings for inputs in the outline; nested inputs use
 *         the same "You" label and collapsed quote box.
...
(function () {
  ...
  const CONFIG = {
    ...
    QUESTION_LABEL: 'Input',        // heading and box title
    QUESTION_HEADING: '#####',      // H5 in the outline ('' to hide the heading)
    QUESTION_BOX: 'quote',          // box style: 'quote' grey, 'note' blue, 'tip' teal, 'example' purple
    QUESTION_FOLD: 'all',           // 'all'  = every question folds under "Input" (tap to open)
  ...
  // A real heading above the callout gives the input an outline entry.
  // Your question stays in a box, collapsed under "Input" by default.
  ...
  function questionBlock(text, files) {
    ...
    const heading = CONFIG.QUESTION_HEADING ? CONFIG.QUESTION_HEADING + ' ' + label + '\n\n' : '';
    if (!CONFIG.CALLOUTS) return heading + '> **' + label + '**\n>\n' + quoted;
    ...
    if (fold === 'none') return heading + head + ' ' + label + '\n' + quoted;
    if (fold === 'all') return heading + head + '- ' + label + '\n' + quoted;
    if (fitsTitle) return heading + head + ' ' + label + ': ' + body;
    return heading + head + '- ' + label + ': ' + previewOf(text, files) + '\n' + quoted;
  }
  ...
  function renderItems(items, ctx, forcedLabel) {
    ...
    items.forEach((m) => {
      if (!m || typeof m !== 'object' || HIDDEN_TYPES.has(m.type) || m.type === 'clear-context') return;
      ...
      // Nested inputs (for example, multi-model follow-ups) use the same
      // "Input" heading and quote box as top-level questions.
      if (m.role === 'user') {
        if (NON_QUESTION_TYPES.has(m.type) && m.type !== 'attachment') return;
        const attachment = m.type === 'attachment';
        const q = questionParts({ user: attachment ? null : m, files: attachment ? [m] : [] });
        if (q.text || q.files.length) chunks.push(questionBlock(q.text, q.files));
        lastLabel = null;
        return;
      }
