...
(function () {
  ...
  // ── Find the response message block for a heading ───────────────────────────
  function findMessageBlock(headingEl) {
    // Walk up from the heading looking for a message-level container.
    // In TypingMind, each AI response lives inside a wrapper that contains
    // both the model avatar area and the rendered markdown content.
    let node = headingEl;
    for (let i = 0; i < 25 && node && node !== document.body; i++) {
      node = node.parentElement;
      if (!node) break;
      // A message block typically has a model avatar img that sits BEFORE
      // the content area. We detect it by checking if this container
      // has both an img and our heading as descendants, and is large enough
      // to be a full message row.
      const rect = node.getBoundingClientRect();
      if (rect.height < 60) continue;
      // Look for an img that's visually above/beside the heading (avatar area)
      const allImgs = node.querySelectorAll('img');
      for (const img of allImgs) {
        const src = img.src || '';
        if (!src || src.startsWith('data:')) continue;
        const imgRect = img.getBoundingClientRect();
        // The model avatar is near the top of the message block,
        // typically small (under 40px rendered), and sits above the heading
        if (imgRect.height > 0 && imgRect.height <= 40 && imgRect.width <= 40
            && imgRect.top < headingEl.getBoundingClientRect().top) {
