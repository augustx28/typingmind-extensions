...
(function () {
  ...
  function findMessageBlock(headingEl) {
    // The response card header has a button with this exact structure:
    //   <button ...>
    //     <span class="relative"><img .../> or <svg .../></span>
    //     <span class="... font-semibold text-xs">Model Name</span>
    //   </button>
    // This pattern is unique to model labels; plugin icons don't have it.
    ...
    for (let i = 0; i < 30 && node && node !== document.body; i++) {
      ...
      // Find all buttons in this container that have a font-semibold text-xs
      // model name label as a direct child span
      const buttons = node.querySelectorAll('button');
      for (const btn of buttons) {
        // Skip tab bar buttons
        if (btn.getAttribute('data-tooltip-id') === 'global') continue;

        // Must have a child span with font-semibold and text-xs (model name)
        const nameSpan = btn.querySelector(':scope > span.font-semibold.text-xs');
        if (!nameSpan) continue;

        // Must appear before the heading in DOM order
        const order = headingEl.compareDocumentPosition(btn);
        if (!(order & 2)) continue; // not preceding

        // Found the model button, now extract the icon
        const img = btn.querySelector('img');
        if (img && img.src) {
          return { container: node, iconSrc: img.src };
        }

        // Check for SVG icon (e.g. Claude)
        ...
        // Button found but no extractable icon
        return { container: node, iconSrc: null };
      }
    }

    // Fallback: single-model mode uses the tab bar
    node = headingEl;
    for (let i = 0; i < 30 && node && node !== document.body; i++) {
      node = node.parentElement;
      if (!node) break;

      const tabBtns = node.querySelectorAll('button[data-tooltip-id="global"][data-tooltip-content]');
      if (tabBtns.length === 0) continue;

      // Only use tab bar in single-model mode
      if (tabBtns.length > 1) return { container: node, iconSrc: null };

      const btn = tabBtns[0];
      const img = btn.querySelector('img');
      if (img && img.src) return { container: node, iconSrc: img.src };
    ...
    return { container: null, iconSrc: null };
  }
