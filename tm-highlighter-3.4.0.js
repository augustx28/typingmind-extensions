...
`;
  ...
  function buildCard(record) {
    ...
    const chatName = titleForRecord(record);

    if (view.scope === "all" && chatName) {
      meta.appendChild(el("span", { class: "tmhl-sep", text: "·" }));
      meta.appendChild(el("span", { class: "tmhl-chat", text: chatName }));
    }
  ...
  function renderSettings() {
    ...
    launcherToggle.addEventListener("click", () => {
      settings.launcherMode = settings.launcherMode === "full" ? "off" : "full";
      saveSettings();
      renderSettings();
      updateLauncher();
    ...
    const autoHideToggle = el("button", {
      ...
      "aria-label": "Hide the launcher while the mobile chat menu is open"
    });
    ...
    /* Sync */
    const syncField = el("div", { class: "tmhl-field" });
    const syncRow = el("div", { class: "tmhl-row" });
    syncRow.appendChild(
      el("span", { class: "tmhl-label", text: "Sync across devices" })
    );
    ...
    const syncToggle = el("button", {
      ...
      class: "tmhl-switch",
      role: "switch",
      "aria-checked": settings.sync.enabled ? "true" : "false",
      "aria-label": "Sync highlights across devices"
    });
    ...
    syncRow.appendChild(syncToggle);
    syncField.appendChild(syncRow);

    const syncButtons = el("div", { class: "tmhl-filters" });

    const tokenButton = el("button", {
      type: "button",
      class: "tmhl-btn",
      text: syncToken() ? "Replace token" : "GitHub token"
    });
    tokenButton.addEventListener("click", connectSync);

    const gistButton = el("button", {
      type: "button",
      class: "tmhl-btn",
      text: "Gist ID"
    });
    gistButton.addEventListener("click", editGistId);

    const copyGistButton = el("button", {
      type: "button",
      class: "tmhl-btn",
      text: "Copy Gist ID"
    });
    copyGistButton.addEventListener("click", async () => {
      if (!settings.sync.gistId) {
        toast("No gist yet. Sync once to create one.");
        return;
      }

      const success = await copyText(settings.sync.gistId);
      toast(success ? "Gist ID copied." : "Copy blocked by the browser.");
    });

    const syncNowButton = el("button", {
      type: "button",
      class: "tmhl-btn",
      text: "Sync now"
    });
    syncNowButton.addEventListener("click", () => syncNow({ force: true }));

    syncButtons.append(tokenButton, gistButton, copyGistButton, syncNowButton);
    syncField.appendChild(syncButtons);

    syncStatusNode = el("div", { class: "tmhl-sync-status", text: "" });
    syncField.appendChild(syncStatusNode);

    syncField.appendChild(
      el("div", {
        class: "tmhl-help",
        text:
          "Highlights are stored in one secret GitHub gist. Create a token at github.com/settings/tokens with the gist scope, paste it here, then use the same Gist ID on your other device. The token stays in this browser and is never put in the gist."
      })
    );
    settingsNode.appendChild(syncField);
    renderSyncStatus();
  ...
  function renderPanel() {
    ...
    if (countNode) {
      ...
      const total = liveItems().length;
      countNode.textContent = view.scope === "chat" ? `${chatCount}` : `${total}`;
    }
  ...
  function layoutLauncher() {
    ...
    const top = clamp(
      ...
      viewport.top + margin,
      Math.max(viewport.top + margin, viewport.bottom - height - margin)
    );
  ...
  function syncLauncherVisibility() {
    ...
    if (!launcherEnabled()) {
      ...
      launcher.classList.remove("tmhl-panel-open", "tmhl-native-sidebar-open");
  ...
  function refreshSidebarVisibilityWatcher() {
    ...
    const shouldObserve = Boolean(document.body) && shouldWatchSidebarVisibility();
  ...
  function onDocumentClick(event) {
    ...
    let rect = location
      ? rangeRect(rangeFromOffsets(location.root, location.start, location.end))
      : null;
  ...
  function onNavigation() {
    ...
    scheduleTitleCapture(700);
    window.setTimeout(syncLauncherVisibility, 300);
    window.setTimeout(() => scheduleTitleCapture(900), 1200);
  }
  ...
  function onStorage(event) {
    ...
    if (event.key === LS_TITLES) {
      chatTitles = loadTitles();
      renderPanel();
    }
  ...
  function initialize() {
    ...
    /* Catch chat titles while the sidebar is open, which is the only
       moment a phone exposes them. */
    document.addEventListener(
      "click",
      (event) => {
        if (
    ...
    const titleObserver = new MutationObserver(() => scheduleTitleCapture(400));
    ...
    scheduleRestore(400);
    scheduleTitleCapture(900);

    if (syncReady()) {
      window.setTimeout(() => syncNow({ silent: true }), 1800);
    }
      ...
      sync: {
        ...
        status() {
          return {
            ...
            hasToken: Boolean(syncToken()),
            gistId: settings.sync.gistId,
