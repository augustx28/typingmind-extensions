 ...
 *  - Compact draggable highlighting toolbar.
 *  - Floating launcher hides while either sidebar is open.
 *  - Avoids false mobile-sidebar detection that hid the whole highlighter.
 *  - Keeps all existing highlights, notes, settings, imports, and exports.
 * ===================================================================== */
...
(() => {
  ...
  const TM_SIDEBAR_OPEN_SELECTOR = [
    ...
    'button[data-element-id="mobile-menu-button"]',
  ...
  const TM_CHAT_NAVIGATION_SELECTOR = [
    'a[href*="#chat="]',
    '[data-chat-id]',
  ...
  function loadSettings() {
    try {
      ...
      if (
        !Number.isFinite(
          launcherPosition.xPct
        ) ||
        !Number.isFinite(
          launcherPosition.yPct
        )
      ) {
        merged.launcher = {
          xPct:
      ...
      merged.toolbar = {
        pinned: Boolean(
          toolbarPosition.pinned
        ),
        xPct: Number.isFinite(
          toolbarPosition.xPct
        )
          ? clamp(
              toolbarPosition.xPct,
              0,
  ...
  function getResponseRoots() {
    ...
    for (
      const selector of ROOT_SELECTORS
    ) {
  ...
  function offsetOfPoint(
    ...
    for (
      const node of getTextNodes(root)
    ) {
      if (node === container) {
        return (
          total +
...
`;
  ...
  function positionToolbarMenu() {
    if (
      !toolbar ||
      !toolbarMenu ||
      !toolbarMenu.classList.contains(
        "tmhl-open"
      )
    ) {
      return;
    ...
    window.requestAnimationFrame(
      () => {
        ...
        const menuWidth =
          toolbarMenu.offsetWidth;

        const menuHeight =
          toolbarMenu.offsetHeight;
  ...
  function setToolbarCoordinates(
    left,
    top
  ) {
    if (
  ...
  function renderSettings() {
    ...
    launcherField.appendChild(
      el("div", {
        class: "tmhl-help",
        text:
          "Off shrinks the launcher to a faint dot. Drag the launcher to reposition it."
      })
    );
    ...
    autoHideRow.appendChild(
      el("span", {
        class: "tmhl-label",
        text:
          "Hide behind chat menu"
      })
    );

    const autoHideToggle = el(
      "button",
      {
        type: "button",
        class: "tmhl-switch",
    ...
    autoHideField.appendChild(
      el("div", {
        class: "tmhl-help",
        text:
          "Hides the floating Highlights button while TypingMind's mobile chat sidebar is open."
      })
    );
    ...
    const positionField = el(
      "div",
      {
        class:
          "tmhl-field tmhl-filters"
      }
    );

    const resetLauncher = el(
      "button",
      {
        type: "button",
        class: "tmhl-btn",
  ...
  function layoutLauncher() {
    ...
    const width =
      launcher.offsetWidth || 62;

    const height =
      launcher.offsetHeight || 34;
  ...
  /* ------------------------------------------------------------------
   * Safe mobile sidebar detection
   * ---------------------------------------------------------------- */

  let nativeSidebarOpen = false;
  let sidebarOpeningHintUntil = 0;
  let sidebarSyncTimer = 0;
  let sidebarObserver = null;

  function explicitSidebarState() {
    if (!isNarrow()) {
      return false;
    }
  ...
  function checkNativeSidebarState() {
    nativeSidebarOpen =
      explicitSidebarState();
  ...
  function scheduleSidebarChecks() {
    [
      0,
      40,
      100,
  ...
  function nodeHasSidebarHook(node) {
  ...
  function mutationAffectsSidebar(
    record
  ) {
    const target =
