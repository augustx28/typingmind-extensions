/* =====================================================================
 * TypingMind - Space Saver  v1.1
 * ---------------------------------------------------------------------
 * More room for the chat on phone and desktop:
 *   1. Header slides away when you scroll down. Scrolling back up does
 *      not reopen it; use the arrow control when you want it back.
 *      It floats over the chat, so hiding it never makes the text jump.
 ...
 *   5. A small chevron sits above the input:
...
html.tss-on:not(.dark)
[data-element-id="chat-space-background"]
div:has(+ [data-element-id="chat-space-middle-part"]) {
  background-color:
    var(--tss-bg, rgba(255,255,255,.92)) !important;
}
...
> .dynamic-chat-content-container {
  padding-top:
    calc(1rem + var(--tss-head, 54px)) !important;
}
...
/* Sticky bars inside the chat sit underneath the header */
...
/* 2. Model + plugin row floats above the input */

html.tss-on.tss-models
...
html.tss-on.tss-models.tss-hide
div:has(+ [data-element-id="chat-space-end-part"]) {
...
html.tss-on.tss-models
[data-element-id="chat-space-middle-part"]
[data-element-id="scroll-padding"] {
  margin-top: var(--tss-row, 44px);
}

/* Fade where chat meets input while row is hidden */

html.tss-on.tss-models
div:has(> [data-element-id="chat-space-end-part"])::before {
  content: "";
...
/* 3. Input tools collapse while box is idle */

html.tss-tools
[data-element-id="chat-space-end-part"]
:not(:focus-within)
:not(:has(#chat-input-textbox:not(:placeholder-shown)))
...
html.tss-dense
[data-element-id="chat-space-end-part"]
> div:has(> [data-element-id="message-input"]) {
  margin-bottom: 10px !important;
}

html.tss-dense.tss-phone
[data-element-id="chat-space-end-part"]
> div:has(> [data-element-id="message-input"]) {
...
html.tss-dense.tss-phone
div:has(> div > [data-element-id="chat-space-end-part"]) {
...
/* Hidden header = downward chevron */
...
/* Fixed/stable mobile positioning */
...
`;
  ...
  /* Finger down, scrollbar drag, wheel, or key input */
  ...
  /* Determine whether scroll was user-driven */
   ...
   * Position arrow.
   *
   * On mobile we no longer calculate the position from the
   * children of the model/plugin row. Those controls can wrap,
   * appear, disappear, or change size, which caused the control
   * to drift horizontally.
   *
   * Instead mobile uses the actual chat input area as its anchor.
   */
      ...
      new ResizeObserver(
        entries => {
          ...
          if (
            ...
            console.warn(
              '[Space Saver] Resize storm detected. Switched to polling.'
            );
  ...
  function tick() {
     ...
     * IMPORTANT CHANGE:
     *
     * Reaching the top no longer forces the header open.
     * This prevents the header appearing while you are
     * simply scrolling upward to reread something.
     */
    ...
    /*
     * Scroll-hide is intentionally one-way.
     *
     * Scroll down = header may hide.
     * Scroll up   = header remains where it is.
     *
     * Use the arrow if you want to restore it.
     */
  ...
  function makeTab() {
    ...
    b.addEventListener(
      ...
      e => {
         ...
         *
         * Otherwise tapping the arrow can blur the
         * message box, collapse input tools and move
         * the footer while your finger is still down.
         */
    ...
    /*
     * Pointer events above normally handle taps.
     *
     * Keep click for keyboard/screen-reader activation.
     */
  ...
  function modeNote(mode) {
    if (
      ...
      return (
        ...
        'Scrolling up keeps it hidden; ' +
        'tap the arrow to bring it back.'
      );
    }
    ...
    if (
      ...
      return isPhone()
        ? 'Stays where you leave it. Tap the arrow to show or hide it.'
        : (
            'Stays where you leave it. ' +
            'Tap the arrow or press ' +
            SHORTCUT +
            '.'
          );
    }
    ...
    return (
      'Normal TypingMind header. ' +
      'Tap the arrow to open these options.'
    );
  }
  ...
  function openSheet() {
    ...
    el.innerHTML =
      ...
      '<div class="tss-seg" role="radiogroup" aria-label="Header">' +
        ...
        '<button type="button" role="radio" data-mode="auto">' +
          'Scroll-hide' +
        '</button>' +

        '<button type="button" role="radio" data-mode="manual">' +
          'Manual' +
        '</button>' +

        '<button type="button" role="radio" data-mode="off">' +
          'Off' +
        '</button>' +
      ...
      '<div class="tss-opt" data-key="models">' +

        '<span>' +
          'Hide model row with header' +
        '</span>' +

        '<button type="button" class="tss-switch" role="switch" aria-label="Hide model row with header">' +
        '</button>' +

      '</div>' +

      '<div class="tss-opt" data-key="tools">' +

        '<span>' +
          'Hide input tools until you tap the box' +
        '</span>' +

        '<button type="button" class="tss-switch" role="switch" aria-label="Hide input tools until you tap the box">' +
        '</button>' +

      '</div>' +

      '<div class="tss-opt" data-key="dense">' +

        '<span>' +
          'Tighter spacing' +
        '</span>' +

        '<button type="button" class="tss-switch" role="switch" aria-label="Tighter spacing">' +
        '</button>' +

      '</div>';

    /*
     * Prevent pressing options from pulling focus out
     * of the TypingMind message box.
     */
  ...
  function maybeHint() {
    ...
    t.textContent =
      isPhone()
        ? (
            'Tap the arrow to show or hide the header. ' +
            'Hold it for options.'
          )
        : (
            'Tap the arrow or press ' +
  ...
  function teardown() {
    ...
    if (
      window.tmSpace &&
      window.tmSpace.version ===
      VERSION
    ) {
      delete window.tmSpace;
    }

    window.__tssTeardown =
      null;

    console.log(
      '[Space Saver] Off until the app reloads.'
    );
  }
