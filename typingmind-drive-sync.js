...
@media (max-width:480px){
  ...
  .tmds-foot .tmds-btn{flex:1 1 auto}
    ...
    /**
     * Keeps a crowded workspace bar readable.
     *
     * Two things go wrong once several extensions add tabs:
     *
     * 1. Each tab gets an equal slice of the bar, and on a phone "Settings"
     *    and "Tweaks" are wider than their slice, so the labels run into each
     *    other ("TweaksSettings"). Adding this extension's tab is what tips a
     *    four-tab bar over the edge, so cleaning up after ourselves is fair.
     *
     * 2. The Tweaks extension styles its label by copying the class list of
     *    the first <span> in a neighbouring tab. In TypingMind's markup that
     *    span is the icon wrapper, so the Tweaks label ends up laid out like
     *    an icon. Tweaks reapplies that on every DOM mutation, so fixing it in
     *    the DOM would start an endless fight between two observers. A
     *    stylesheet wins quietly instead.
     *
     * Nothing here is destructive: it only clamps labels to their own tab and
     * tightens type slightly, and only once the bar is genuinely crowded.
     */
    applyBarCompat() {
      const bar = this.button?.parentElement;
      if (!bar) return;
      bar.classList.add("tmds-tabbar");

      const tabs = Array.from(bar.children).filter(
        (el) => el.tagName === "BUTTON" || el.tagName === "A"
      );
      const hasTweaks = !!document.getElementById("workspace-tab-tweaks");
      const rules = [];

      // Safe on any bar: let tabs shrink, and anchor absolutely-positioned
      // children to their own tab rather than to the whole bar.
      rules.push(".tmds-tabbar > button,.tmds-tabbar > a{min-width:0;position:relative}");

      // Undo the Tweaks label inheriting icon layout. Listed before the
      // compaction rule so compaction's typography still wins.
      if (hasTweaks) {
        rules.push(
          "#workspace-tab-tweaks > span{display:block;width:100%;" +
            "text-align:center;align-self:stretch}"
        );
      }

      if (tabs.length >= 5) {
        // :has() lets us target the text label and leave icon wrappers alone,
        // so a status dot or badge inside an icon never gets clipped.
        const label =
          ".tmds-tabbar > button > span:not(:has(svg)):not(:has(img))," +
          ".tmds-tabbar > a > span:not(:has(svg)):not(:has(img))" +
          (hasTweaks ? ",#workspace-tab-tweaks > span:not(:has(svg))" : "");
        rules.push(
          label +
            "{display:block;max-width:100%;padding:0 2px;font-size:0.66rem;" +
            "line-height:0.95rem;letter-spacing:-0.01em;white-space:nowrap;" +
      ...
      // Idempotent: only touch the DOM when the rules actually change, so this
      // can safely run from a MutationObserver.
      if (el.textContent !== css) el.textContent = css;
    },

    /**
     * Other extensions add their tabs whenever they get around to it, which is
     * usually after we mount. Watch the bar itself so the guard applies as soon
     * as the bar changes shape, not just when TypingMind repaints Settings.
     */
    watchBar() {
      const bar = this.button?.parentElement;
