(function () {
  ...
  const settingsKeys = {
    ...
    globalUiFontSize: "tweak_globalUiFontSize",
  ...
  const UI_SIZE_KEYS = [
    settingsKeys.globalUiFontSize,
    settingsKeys.sidebarFontSize,
  ];
  ...
  // ---------------------------------------------------------------
  // UI font sizes (menu bar chat list + whole app)
  // ---------------------------------------------------------------
  function applyUiFontSizes() {
    const styleId = "tweak-ui-font-size-style";
    ...
    const readSize = (rawValue, min, max) => {
      if (rawValue === null || typeof rawValue === "undefined" || String(rawValue).trim() === "") return null;
      const parsed = parseInt(rawValue, 10);
      if (isNaN(parsed)) return null;
      return Math.min(max, Math.max(min, parsed));
    };
    const globalSize = readSize(getSetting(settingsKeys.globalUiFontSize, null), 10, 30);
    const sidebarSize = readSize(getSetting(settingsKeys.sidebarFontSize, null), 8, 40);
    const rules = [];

    if (globalSize) {
      // The app sizes its text and spacing in rem, so the root size scales everything.
      rules.push(`html { font-size: ${globalSize}px !important; }`);
    }
    if (sidebarSize) {
      const selectors = [];
  ...
  function createSettingsModal() {
    ...
    uiSizeHeader.textContent = "UI font size";
    uiSizeSection.appendChild(uiSizeHeader);
    const uiSizeNote = document.createElement("p");
    uiSizeNote.className = "tweak-section-note";
    uiSizeNote.textContent = "Make the menu text bigger. Leave a box empty to keep the TypingMind default.";
    uiSizeSection.appendChild(uiSizeNote);

    const sidebarSizeSubTitle = document.createElement("div");
    sidebarSizeSubTitle.className = "font-customization-subsection-title";
    sidebarSizeSubTitle.textContent = "Menu bar chat list";
    sidebarSizeSubTitle.style.marginTop = "4px";
    uiSizeSection.appendChild(sidebarSizeSubTitle);
    const sidebarSizeDescription = document.createElement("p");
    sidebarSizeDescription.className = "tweak-sub-note";
    sidebarSizeDescription.textContent = "Chat titles and folders in the left sidebar. Allowed range: 8 to 40.";
    uiSizeSection.appendChild(sidebarSizeDescription);
    ...
    const globalSizeSubTitle = document.createElement("div");
    globalSizeSubTitle.className = "font-customization-subsection-title";
    globalSizeSubTitle.textContent = "Whole app";
    uiSizeSection.appendChild(globalSizeSubTitle);
    const globalSizeDescription = document.createElement("p");
    globalSizeDescription.className = "tweak-sub-note";
    globalSizeDescription.textContent = "Root size for every menu, button and label. 16 is the default; higher values scale the interface up. Allowed range: 10 to 30.";
    uiSizeSection.appendChild(globalSizeDescription);
    const globalUiFontSizeInput = createTextInput("tweak_globalUiFontSize_input", "App size (px):", settingsKeys.globalUiFontSize, "e.g., 17", "number", {
      min: "10", max: "30", step: "1", inputmode: "numeric"
    });
    globalUiFontSizeInput.style.marginTop = "5px";
    uiSizeSection.appendChild(globalUiFontSizeInput);
  ...
  function loadSettingsIntoModal() {
    ...
    const globalUiSizeValue = getSetting(settingsKeys.globalUiFontSize, null);
    document.getElementById("tweak_globalUiFontSize_input").value = globalUiSizeValue !== null ? globalUiSizeValue : "";
  ...
  function saveSetting(key, value) {
    try {
      ...
      } else if ([settingsKeys.customFontSize, settingsKeys.globalUiFontSize, settingsKeys.sidebarFontSize].includes(key)) {
      ...
      if (UI_SIZE_KEYS.includes(key)) applyUiFontSizes();
