 ...
 *
 * What's new vs v2
 *  - Highlights panel (drawer on desktop, bottom sheet on phone) that
 *    lists every highlight, searches them, and jumps to them.
 *  - Notes attached to any highlight.
 *  - Cleaner marks: soft tint + bottom stroke, no layout shift, theme aware.
 *  - Real cross-device sync through any JSON endpoint you own (optional).
 *  - Cross-tab live updates, tombstoned deletes, safe merges.
 *  - Selection anchoring is more accurate: buttons and KaTeX mirrors are
 *    excluded from text offsets, plus a whitespace-tolerant fallback match.
...
(() => {
  ...
  function persist(options) {
    ...
    if (!opts.skipSync) queueSyncPush();
    renderPanel();
    updateLauncher();
    return true;
  }
  ...
  /* ------------------------------------------------------------------
   * Remote sync (optional, bring your own endpoint)
   * ---------------------------------------------------------------- */
  ...
  function syncHeaders() {
    ...
    const raw = (settings.sync.headers || "").trim();
    if (!raw) return base;
    try {
      const parsed = JSON.parse(raw);
  ...
  async function syncNow(reason) {
    if (!settings.sync.enabled || !settings.sync.url) {
      sync.state = "off";
      renderSyncStatus();
      return false;
    }
    ...
    try {
      const response = await fetch(settings.sync.url, {
        method: "GET",
        headers: syncHeaders(),
        cache: "no-store"
      });
      ...
      if (response.ok) {
        ...
        if (text.trim()) {
          const parsed = unwrapRemote(JSON.parse(text));
      ...
      const localWins =
        (store.updatedAt || 0) > (remote.updatedAt || 0) || merged.changed;
      ...
      if (localWins) {
        const push = await fetch(settings.sync.url, {
          method: "PUT",
          headers: syncHeaders(),
          body: JSON.stringify({
            version: 3,
        ...
        if (!push.ok) throw new Error(`PUT ${push.status}`);
      }
      ...
      sync.state = "error";
      sync.lastError = String(error && error.message ? error.message : error);
      if (reason === "manual") toast(`Sync failed: ${sync.lastError}`);
      return false;
    } finally {
      sync.inFlight = false;
      renderSyncStatus();
  ...
  function queueSyncPush() {
    if (!settings.sync.enabled || !settings.sync.url) return;
    window.clearTimeout(sync.pushTimer);
    sync.pushTimer = window.setTimeout(() => syncNow("auto"), 2500);
  }
  ...
  function restartSyncTimer() {
    ...
    const seconds = clamp(Number(settings.sync.intervalSec) || 60, 15, 3600);
    sync.timer = window.setInterval(() => syncNow("interval"), seconds * 1000);
    syncNow("startup");
  }
...
#tmhl-launcher:hover { opacity: 1; transform: translateY(-1px); }
...
@media (max-width: 720px) {
  ...
  #tmhl-launcher { bottom: calc(78px + env(safe-area-inset-bottom, 0px)); right: 12px; }
}
...
`;
  ...
  function positionToolbar(rect) {
    ...
    requestAnimationFrame(() => {
      ...
      if (isTouch()) {
        top = below + height < vh - 8 ? below : Math.max(8, above);
      } else {
  ...
  function buildPanel() {
    ...
    controls.setAttribute("data-tmhl-controls", "true");
    panel.dataset.controls = "1";
  ...
  function renderSettings() {
    ...
    toggle.addEventListener("click", () => {
      settings.sync.enabled = !settings.sync.enabled;
      saveSettings();
      renderSettings();
      restartSyncTimer();
    });
    syncRow.appendChild(toggle);
    syncField.appendChild(syncRow);
    syncField.appendChild(
      el("div", {
        class: "tmhl-help",
        text:
          "Point this at any JSON endpoint you control that answers GET and PUT. A free Cloudflare Worker takes about five minutes. Everything keeps working locally if you leave this off."
      })
    );
    settingsNode.appendChild(syncField);

    const urlField = el("div", { class: "tmhl-field" });
    urlField.appendChild(el("label", { class: "tmhl-label", text: "Endpoint URL" }));
    const urlInput = el("input", {
      class: "tmhl-input",
      type: "url",
      placeholder: "https://highlights.your-worker.workers.dev/store"
    });
    urlInput.value = settings.sync.url || "";
    urlInput.addEventListener("change", () => {
      settings.sync.url = urlInput.value.trim();
      saveSettings();
      restartSyncTimer();
    });
    urlField.appendChild(urlInput);
    settingsNode.appendChild(urlField);

    const headerField = el("div", { class: "tmhl-field" });
    headerField.appendChild(
      el("label", { class: "tmhl-label", text: "Extra headers (JSON)" })
    );
    const headerInput = el("textarea", {
      class: "tmhl-input",
      placeholder: '{"Authorization": "Bearer your-token"}'
    });
    headerInput.value = settings.sync.headers || "";
    headerInput.addEventListener("change", () => {
      settings.sync.headers = headerInput.value.trim();
      saveSettings();
    });
    headerField.appendChild(headerInput);
    settingsNode.appendChild(headerField);

    const intervalField = el("div", { class: "tmhl-field" });
    intervalField.appendChild(
      el("label", { class: "tmhl-label", text: "Check every (seconds)" })
    );
    ...
    intervalInput.value = String(settings.sync.intervalSec || 60);
    intervalInput.addEventListener("change", () => {
      settings.sync.intervalSec = clamp(Number(intervalInput.value) || 60, 15, 3600);
      saveSettings();
      restartSyncTimer();
    });
    intervalField.appendChild(intervalInput);
    settingsNode.appendChild(intervalField);

    const syncActions = el("div", { class: "tmhl-field tmhl-filters" });
    const syncBtn = el("button", { class: "tmhl-btn", text: "Sync now" });
    syncBtn.addEventListener("click", () => syncNow("manual"));
    syncActions.appendChild(syncBtn);
    settingsNode.appendChild(syncActions);

    const launcherField = el("div", { class: "tmhl-field" });
    const launcherRow = el("div", { class: "tmhl-row" });
    launcherRow.appendChild(
      el("span", { class: "tmhl-label", text: "Show floating button" })
    );
    ...
    launcherField.appendChild(
      el("div", {
        class: "tmhl-help",
        text: "Alt+L opens this panel either way. Alt+H highlights the selection with your last color."
      })
    );
  ...
  function renderSyncStatus() {
    ...
    if (!settings.sync.enabled || !settings.sync.url) {
      statusNode.textContent = "Local only";
      return;
    }
    if (sync.state === "syncing") statusNode.textContent = "Syncing…";
    else if (sync.state === "error") statusNode.textContent = "Sync error";
    else if (sync.lastAt) {
      statusNode.textContent = `Synced ${relativeTime(
        new Date(sync.lastAt).toISOString()
      )}`;
    } else statusNode.textContent = "Sync on";
  }
  ...
  function initialize() {
    ...
    window.TMHighlighter = {
      ...
      setSync: (config) => {
        ...
        restartSyncTimer();
        renderPanel();
        return structuredCopy(settings.sync);
      },
