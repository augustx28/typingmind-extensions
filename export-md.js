/* ==========================================================================
 * TypingMind Chat Export (.md)  v1.3
 * --------------------------------------------------------------------------
 ...
 *   as the model wrote them. Each question sits in a grey box; each answer
...
(function () {
  ...
  const CONFIG = {
    ...
    QUESTION_FOLD: 'long',          // 'long' = only long questions fold up (tap to open)
                                    // 'all'  = every question folds up
                                    // 'none' = never fold, full question always showing
    QUESTION_TITLE_CHARS: 90        // a question up to this long (one line) fits on the box title
  };
  ...
  // Your question in a callout box. A short one-line question sits right on
  // the box's title line. A longer one folds up under a preview of its first
  // line; tap the box to open it.
  function questionBlock(text, files) {
    ...
    if (fold !== 'all' && fitsTitle) return head + ' ' + label + ': ' + body;
