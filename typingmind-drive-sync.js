...
(function () {
  ...
  const Engine = {
    ...
    async sync({ deep = false, reason = "auto" } = {}) {
      ...
      if (!this.configured()) {
        this.emit("idle", "Not configured yet.");
        return;
      }
      this.running = true;
      this.lastError = "";
  ...
  const UI = {
    ...
    injectStyles() {
      ...
      s.textContent = `
#tmds-dot{position:absolute;top:-3px;right:-6px;width:8px;height:8px;border-radius:50%;
  background:#71717a;z-index:5;box-shadow:0 0 0 2px rgba(0,0,0,.35)}
...
<div id="tmds-panel" role="dialog" aria-modal="true" aria-label="Drive Sync">
  ...
  <div class="tmds-foot">
    ...
    wire(root) {
      ...
      $("tmds-signin").onclick = (e) =>
        busy(e.target, "Opening Google…", async () => {
          writeCfg(K.clientId, $("tmds-clientid").value.trim());
          Drive.tokenClient = null;
          await Drive.auth({ interactive: true });
          Engine.emit("ok", "Signed in to Google.");
          await this.loadSnapshots();
        });

      $("tmds-now").onclick = (e) =>
        busy(e.target, "Syncing…", () => Engine.sync({ reason: "manual" }));

      $("tmds-verify").onclick = (e) =>
        busy(e.target, "Verifying…", () => Engine.sync({ deep: true, reason: "verify" }));

      $("tmds-snap-refresh").onclick = (e) =>
        busy(e.target, "Loading…", async () => {
          await Drive.buildIndex();
          await this.loadSnapshots();
        });
    ...
    refreshPanel() {
      ...
      if (msg) {
        msg.textContent =
          Engine.status === "auth"
            ? `${Engine.message} Use "Sign in to Google" above.`
  ...
  async function boot() {
    ...
    if (!Engine.configured()) {
      Engine.emit("idle", "Open Sync to add a Google Client ID and passphrase.");
      return;
    }
