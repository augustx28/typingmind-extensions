(() => {
  ...
  /*
   * Lighter colors designed for improved text visibility,
   * especially with TypingMind's dark theme.
   */
  const COLORS = {
    yellow: "rgba(250, 204, 21, 0.28)",
    green: "rgba(74, 222, 128, 0.22)",
    blue: "rgba(96, 165, 250, 0.24)",
    pink: "rgba(244, 114, 182, 0.23)"
  };
  ...
  /*
   * DOM mark rendering is intentionally enabled.
   * This allows rounded highlight corners.
   */
  const supportsCustomHighlights = false;
  ...
  function readState() {
    try {
      ...
      console.warn(
        "[TypingMind Highlighter] Could not read storage.",
        error
      );
  ...
  function rangeFromOffsets(root, start, end) {
    ...
    const startPoint = locateTextPoint(
      root,
      start
    );

    const endPoint = locateTextPoint(
      root,
      end
    );
  ...
  function resolveRecord(record, roots) {
    ...
    for (const info of rootInfo) {
      ...
      for (const start of positions) {
        ...
        if (
          info.index === record.messageIndex
        ) {
          score += 800;
        }
        ...
        if (record.prefix) {
          const before = info.text.slice(
            Math.max(
              0,
              start - record.prefix.length
            ),
            start
          );

          if (
            before.endsWith(record.prefix)
          ) {
            score += 2500;
          }
        }
        ...
        if (record.suffix) {
          ...
          if (
            after.startsWith(record.suffix)
          ) {
            score += 2500;
          }
        }
  ...
  function getFallbackMarksById(id) {
    return Array.from(
      document.querySelectorAll(
        FALLBACK_MARK_SELECTOR
      )
      ...
      (mark) =>
        mark.dataset.tmphId === id
    );
  }

  function fallbackMarkExists(id) {
    return (
      getFallbackMarksById(id).length > 0
    );
  }
  ...
  function wrapFallbackRange(
    ...
    try {
      for (
        ...
        mark.dataset.tmphChat =
          record.chatId;

        mark.dataset.tmphColor =
          record.color;
        ...
        mark.style.borderRadius = "0.32em";
  ...
  function renderWithFallbackMarks(
    ...
    clearFallbackMarks(
      (mark) =>
        mark.dataset.tmphChat !== chatId
    );

    for (const location of locations) {
      const existingMarks =
        getFallbackMarksById(
          location.record.id
        );
  ...
  function restoreHighlights() {
    ...
    const locations = records
      .map((record) =>
        resolveRecord(record, roots)
      )
      .filter(Boolean);

    renderedHighlights = locations;

    renderWithFallbackMarks(
      locations,
      chatId
    );
  }
  ...
  function selectionOverlapsRendered(
    ...
    return renderedHighlights.find(
      (location) => {
        return (
          location.root === root &&
          start < location.end &&
  ...
  function createHighlight(color) {
    ...
    if (
      ...
      showToast(
        "The chat changed. Select the text again."
      );
    ...
    if (messageIndex < 0) {
      ...
      showToast(
        "Could not identify this response."
      );
  ...
  function recolorHighlight(
    ...
    /*
     * Remove the old rendered marks so they
     * can be rebuilt with the new color.
     */
    clearFallbackMarks(
      (mark) =>
        mark.dataset.tmphId === id
  ...
  function importHighlights(input) {
    try {
      ...
      if (!Array.isArray(items)) {
        throw new Error(
          "Invalid highlight file."
        );
      }

      const validItems =
        items.filter(isValidRecord);
  ...
  function initialize() {
    ...
    console.info(
      `[TypingMind Highlighter] Version ${VERSION} loaded. Rounded highlight mode enabled.`
    );
  }
