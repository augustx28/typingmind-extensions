 ...
 * TypingMind Tweaks
 *
 * Patched 2026-07-25 — workspace bar tab layout.
 *
 * Three related fixes, all inside syncTweaksButton() and its helpers:
 *
 *   1. The label span was styled by copying the class list of
 *      referenceButton.querySelector("span"). TypingMind wraps each tab icon
 *      in its own <span>, so that returned the ICON WRAPPER, not the text
 *      label. The Tweaks label therefore inherited icon layout instead of the
 *      label's width, centering and type classes, and ran into the next tab's
 *      label ("TweaksSettings"). It now picks the span that actually contains
 *      text.
 *
 *   2. The button was built as <button><svg><span>, while every TypingMind tab
 *      is <button><span><svg></span><span>. Different structure means a
 *      different width in the flex row, which is the other half of the
 *      collision. The icon is now wrapped to match, and the wrapper mirrors
 *      the reference wrapper's classes.
 *
 *   3. Active-state classes were rewritten with String.replace, but
 *      "sm:hover:bg-white/20" contains "bg-white/20" as a substring, so a tab
 *      carrying both could be corrupted into "sm:hover:sm:hover:bg-white/20".
 *      Class rewriting is now done on whole tokens.
 *
 * Also: getReferenceButton no longer prefers another extension's Sync tab. It
 * mirrors one of TypingMind's own tabs, so the structure being copied is the
 * real thing rather than another extension's approximation of it.
 *
...
(function () {
  ...
  function applyStylesBasedOnSettings() {
    ...
    const wsIconColor = cleanValue(getSetting(settingsKeys.workspaceIconColor, null));
    ...
    if (workspaceBar) {
      // The Tweaks icon is no longer special-cased. It used to be forced to
      // #9ca3af whenever no custom colour was set, while every other icon
      // inherited text-white/70 -- which is why this one tab looked a slightly
      // different shade. Every icon in the bar is now treated identically:
      // use the chosen colour, or inherit when there isn't one.
  ...
  // FIX: mirror one of TypingMind's own tabs.
  //
  // This used to prefer another extension's Sync tab, on the theory that it
  // was unlikely to be in an active state. But that meant copying an
  // approximation of a native tab instead of the real thing, and inheriting
  // whatever markup that extension happened to use. The active state is
  // already handled below, so there is no reason to avoid the real tabs.
  function getReferenceButton(workspaceBar) {
    ...
    return (
      settingsBtn ||
      chatBtn ||
      // Last resort: any other workspace tab, but never this one.
      workspaceBar.querySelector('button[data-element-id^="workspace-tab-"]:not(#workspace-tab-tweaks)')
    );
  }

  // FIX: identify a tab's parts properly.
  //
  // TypingMind builds every workspace tab as:
  //   <button>
  //     <span class="...icon wrapper..."><svg/></span>
  //     <span class="...label classes...">Label</span>
  //   </button>
  //
  // The old code used referenceButton.querySelector("span"), which returns the
  // FIRST span -- the icon wrapper. Copying its classes onto the Tweaks label
  // meant the label was laid out like an icon: no stretch, no centering, wrong
  // type size. On a crowded bar that is what made "Tweaks" run into the label
  // of the tab beside it.
  function getButtonParts(button) {
    const svg = button.querySelector("svg");
    // Only treat it as a wrapper if the icon is actually nested in something.
    const iconWrap = svg && svg.parentElement && svg.parentElement !== button
      ? svg.parentElement
      : null;
    const label =
      Array.from(button.querySelectorAll("span")).find(
        (el) => !el.querySelector("svg") && el.textContent && el.textContent.trim()
      ) || null;
    return { svg, iconWrap, label };
  }
  ...
  function syncTweaksButton() {
    ...
    if (!tweaksButton) {
      ...
      // TypingMind puts the <svg> straight into the tab button -- there is no
      // wrapper element around it. Matching that exactly keeps this tab the
      // same width as every other one in the flex row.
    ...
    // 2. Mirror the reference button's classes.
    //
    // FIX: work on whole class tokens rather than doing string replacement.
    // "sm:hover:bg-white/20" contains "bg-white/20" as a substring, so
    // String.replace could rewrite the wrong one and produce nonsense like
    // "sm:hover:sm:hover:bg-white/20". Token comparison can't do that.
    const refClasses = referenceButton.className.split(/\s+/).filter(Boolean);
    const isTweaksActive = modalOverlay && window.getComputedStyle(modalOverlay).display !== "none";
    const isRefActive = refClasses.includes("bg-white/20");

    let targetClasses = refClasses;
    if (isRefActive && !isTweaksActive) {
      // The tab we're copying is the selected one; render ours as unselected.
      targetClasses = refClasses.map((c) =>
        c === "bg-white/20" ? "sm:hover:bg-white/20" : c === "text-white" ? "text-white/70" : c
      );
    } else if (!isRefActive && isTweaksActive) {
      // Our modal is open; render ours as selected.
      targetClasses = refClasses.map((c) =>
        c === "sm:hover:bg-white/20" ? "bg-white/20" : c === "text-white/70" ? "text-white" : c
      );
    }
    ...
    // 3. Mirror the reference button's internal parts, matched by role rather
    //    than by document order.
    const ref = getButtonParts(referenceButton);
    const mine = getButtonParts(tweaksButton);

    // If an older build of this script left a wrapper around the icon, make it
    // transparent to layout so the icon still behaves as a direct child.
    if (mine.iconWrap) {
      if (mine.iconWrap.getAttribute("class")) mine.iconWrap.setAttribute("class", "");
      if (mine.iconWrap.style.display !== "contents") mine.iconWrap.style.display = "contents";
    }

    if (mine.svg) {
      const refSvgClass = (ref.svg && ref.svg.getAttribute("class")) || "w-4 h-4 flex-shrink-0";
      if (mine.svg.getAttribute("class") !== refSvgClass) {
        mine.svg.setAttribute("class", refSvgClass);
      }
      // FIX: TypingMind's icons also carry width/height attributes alongside
      // the w-4/h-4 classes. Mirroring them keeps the icon sized correctly even
      // if those utility classes are ever unavailable, rather than falling back
      // to an SVG's 300x150 default.
      if (ref.svg) {
        ["width", "height"].forEach((attr) => {
          const value = ref.svg.getAttribute(attr);
          if (value && mine.svg.getAttribute(attr) !== value) mine.svg.setAttribute(attr, value);
        });
      }
      // FIX: inherit when no colour is chosen, exactly like every native icon.
      // This used to force #9ca3af whenever the setting was unset, which made
      // this one tab a visibly different shade from all its neighbours.
      const customIconColor = cleanValue(getSetting(settingsKeys.workspaceIconColor, null));
      const desiredColor = customIconColor || "";
      if (mine.svg.style.color !== desiredColor) mine.svg.style.color = desiredColor;
    }

    if (mine.label) {
      if (ref.label) {
        const refLabelClass = ref.label.getAttribute("class") || "";
        if (mine.label.getAttribute("class") !== refLabelClass) {
          mine.label.setAttribute("class", refLabelClass);
        }

        // FIX -- this is the one that causes "TweaksSettings" on phones.
        //
        // TypingMind keeps real layout behaviour in an INLINE style on its tab
        // labels, not only in classes:
        //     style="hyphens: auto; word-break: break-word;"
        //
        // Tabs are min-w-[58px] with px-0.5, so a label gets about 54px and is
        // w-full + text-center. A word wider than that only wraps because of
        // that inline word-break. Copying the class but not the style left this
        // label unable to wrap, so it spilled sideways over the next tab.
        //
        // Visibility is folded into the same string and written once. Setting
        // the attribute and then style.display separately would leave the
        // attribute differing from the reference on the next tick, and this
        // script's own MutationObserver would rewrite it forever.
        const refStyle = ref.label.getAttribute("style") || "";
        const refHidden = window.getComputedStyle(ref.label).display === "none";
        const desiredStyle = refHidden
          ? (refStyle ? refStyle.replace(/\s*;?\s*$/, "; ") : "") + "display: none;"
          : refStyle;
        if (mine.label.getAttribute("style") !== desiredStyle) {
          mine.label.setAttribute("style", desiredStyle);
        }
      } else {
        // Reference tab shows no label at all (icon-only rail): hide ours.
        if (mine.label.style.display !== "none") mine.label.style.display = "none";
      }
    }
