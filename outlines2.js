...
//
// v4.4 changes:
// - Model icons no longer get a forced background color. Icons sit in a
//   16x16 box with object-fit: contain, so any non-square logo was
//   letterboxed and the forced white background showed as a bright strip
//   down the side. The icon is transparent now, so only the logo paints.
// - The gold accent is gone. User input rows, the active toggle state,
//   and focus rings use a neutral grey that reads the same in light and
//   dark mode. The jump-to-heading flash inside the chat stays yellow,
//   since a grey flash disappears against a message background.
//
// v4.3 changes:
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      #${TOGGLE_ID}:focus-visible {
        outline: 2px solid rgba(120, 120, 130, 0.75);
        outline-offset: 2px;
      }
      ...
      #${PANEL_ID} .outline-item:focus-visible {
        outline: 1px solid rgba(120, 120, 130, 0.7);
        outline-offset: -1px;
      ...
      #${PANEL_ID} .outline-prompt {
        ...
        border-left-color: rgba(120, 120, 130, 0.45);
      }
      ...
      #${PANEL_ID} .outline-prompt:hover {
        ...
        border-left-color: rgba(90, 90, 100, 0.7);
      }
