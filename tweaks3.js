(function () {
  ...
  const UPLOADED_CHAT_FONT_FAMILY = 'TMTweaksUploadedChatFont';
  ...
  function updateChatFontFileStatus() {
    const statusRow = document.getElementById("tweak_chatFontFile_status");
    const nameEl = document.getElementById("tweak_chatFontFile_name");
    if (!statusRow || !nameEl) return;
    const fileData = getSetting(settingsKeys.chatFontFile, null);
    if (fileData && fileData.dataUrl) {
      const localSet = !!cleanValue(getSetting(settingsKeys.localFontFamily, null));
      const urlFamilySet = !!cleanValue(getSetting(settingsKeys.customFontFamily, null));
      const inUse = !localSet && !urlFamilySet;
      nameEl.textContent = `${fileData.fileName || "Uploaded font"}${inUse ? " \u2014 in use" : " \u2014 stored (not in use)"}`;
      statusRow.style.display = "flex";
    } else {
      nameEl.textContent = "";
  ...
  function createSettingsModal() {
    ...
    chatFileSubTitle.textContent = "Font file upload";
    ...
    chatFileGroup.innerHTML = `
        <label for="tweak_chatFontFile_input">Upload a font from your device:</label>
        <input type="file" id="tweak_chatFontFile_input" accept=".ttf,.otf,.woff,.woff2">
        <div class="${SCRIPT_PREFIX}font-file-status" id="tweak_chatFontFile_status">
            <span id="tweak_chatFontFile_name"></span>
            <button type="button" class="tweak-reset-button" id="tweak_chatFontFile_remove">Remove</button>
        </div>
        <small>.ttf, .otf, .woff or .woff2 \u2014 applied instantly and stored only in this browser. Max 4 MB (.woff2 is smallest). Clearing the fields above hands the chat back to this font.</small>
    `;
    ...
    const chatFontFileRemoveButton = document.getElementById("tweak_chatFontFile_remove");

    chatFontFileInput.addEventListener("change", (event) => {
      const file = event.target.files[0];
      if (!file) return;
      const format = getFontFormatFromFileName(file.name);
      if (!format) {
      ...
      const reader = new FileReader();
      reader.onload = (e) => {
        saveSetting(settingsKeys.chatFontFile, {
          fileName: file.name,
          dataUrl: e.target.result,
          format: format,
        });
        const stored = getSetting(settingsKeys.chatFontFile, null);
        if (!stored || !stored.dataUrl) {
          // Storage failed (saveSetting already showed the error).
          chatFontFileInput.value = "";
          updateChatFontFileStatus();
          return;
        }
        // Clear the other chat font sources so the file takes effect right away.
        ...
        const localEl = document.getElementById("tweak_localFontFamily_input");
        const urlEl = document.getElementById("tweak_customFontUrl_input");
        const familyEl = document.getElementById("tweak_customFontFamily_input");
        if (localEl) localEl.value = "";
        if (urlEl) urlEl.value = "";
        if (familyEl) familyEl.value = "";
        updateChatFontFileStatus();
        showFeedback(`Chat font "${file.name}" applied.`);
      };
      reader.onerror = () => {
        showFeedback("Error reading font file.", 2500);
        console.error(`${consolePrefix} Error reading chat font file.`);
      };
      reader.readAsDataURL(file);
    });

    chatFontFileRemoveButton.addEventListener("click", () => {
      saveSetting(settingsKeys.chatFontFile, null);
      chatFontFileInput.value = "";
      updateChatFontFileStatus();
      showFeedback("Uploaded chat font removed.");
    });
  ...
  function applyCustomFont() {
    ...
    // Uploaded chat font file: used when no local or URL font is set above.
    const chatFontFileData = getSetting(settingsKeys.chatFontFile, null);
    const chatFileReady = syncUploadedFontFace(
      chatFontFileData && chatFontFileData.dataUrl ? chatFontFileData : null,
      "tweak-uploaded-chat-font-face",
      UPLOADED_CHAT_FONT_FAMILY
    );
    let effectiveFontFamily = cleanedLocalFamily || cleanedFamilyFromUrl || (chatFileReady ? UPLOADED_CHAT_FONT_FAMILY : null);
