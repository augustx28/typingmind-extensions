/* ==========================================================================
 * TypingMind Chat Export (.md)  v1.1
 * --------------------------------------------------------------------------
 * Exports the chat you have open to one Markdown file with only:
 *   the chat title, the agent and/or model, and every question and answer
 *   in order, links exactly as the model wrote them.
 ...
 *
 * Use
 *   "Export" tab in the workspace bar (next to Settings), or
 *   Ctrl/Cmd + Shift + E to download the open chat instantly.
 *   The Export menu also has Copy, Share (phone) and three toggles:
...
(function () {
  ...
  const CONFIG = {
    ...
    DEFAULTS: { tools: false, thinking: false, system: false }
  };
  ...
  // True when a reply shows something besides tool calls (text or an image).
  function hasVisibleReply(m) {
    if (typeof m.content === 'string') return !!m.content.trim();
    if (!Array.isArray(m.content)) return false;
    return m.content.some((b) => {
      if (typeof b === 'string') return !!b.trim();
      if (!b || typeof b !== 'object') return false;
      if (b.type === 'text' || b.type === 'output_text') return !!String(b.text || '').trim();
      return !!imageLine(b);
    });
  }
  ...
  // Models that wrote a visible reply, in order of first appearance.
  function modelsUsed(messages, chat) {
    const list = [];
    const add = (id) => {
      const label = cleanLabel(modelLabel(id, chat));
    ...
    messages.forEach((m) => {
      ...
      if (m.type === 'tm_multi_responses' && Array.isArray(m.responses)) {
        m.responses.forEach((r) => {
          ...
          if (msgs.some((x) => x && x.role === 'assistant' && hasVisibleReply(x))) add(r.model);
        });
        return;
      }
      if (m.role === 'assistant' && hasVisibleReply(m)) add(m.model || chat.model);
    });
    return list;
  }

  // Replies are labeled with the agent, or the model when there is no agent.
  // The model is added next to the agent only when the chat used more than one.
  function labelFor(modelId, ctx) {
    const model = cleanLabel(modelLabel(modelId, ctx.chat));
    if (ctx.agent) return ctx.multiModel && model ? ctx.agent + ' · ' + model : ctx.agent;
  ...
  function renderItems(items, ctx, forcedLabel) {
    ...
    items.forEach((m) => {
      ...
      if (text.trim()) {
        ...
        chunks.push('**' + cleanLabel(role.charAt(0).toUpperCase() + role.slice(1)) + '**', finalize(text));
        lastLabel = null;
      }
    });
  ...
  // The whole file: title, who answered, then each question and its answer.
  function buildMarkdown(chat, opts) {
    ...
    ctx.models = modelsUsed(messages, chat);
    ctx.multiModel = ctx.models.length > 1;
    ...
    groupTurns(messages).forEach((turn) => {
      ...
      if (turn.user) {
        questions++;
        const body = [escapeHeadings(escapeTags(q.text)), ...q.files].filter(Boolean).join('\n\n') || '*[empty message]*';
        parts.push('**You**', closeFences(body));
    ...
    const who = [];
    if (ctx.agent) who.push('**Agent:** ' + ctx.agent);
    if (ctx.models.length) who.push('**Model:** ' + ctx.models.join(', '));
    if (who.length) head.push(who.join(' · '));
  ...
  function chatFromScreen() {
    ...
    blocks.forEach((b) => {
      ...
      if (u) {
        ...
        // A reply block before the first question is the (visible) system prompt.
        if (md.trim()) messages.push({ role: seenUser ? 'assistant' : 'system', content: md, _fromScreen: true });
      }
    });
  ...
  function tick() {
    ...
    try {
      if (syncButton()) {
        S.barSeen = true;
        removeFab();
      } else if (!S.barSeen && Date.now() - S.startedAt > CONFIG.FAB_AFTER_MS && document.querySelector(SEL.chatSpace)) {
        ensureFab();
      }
    } catch (e) {
  ...
  const api = {
    ...
    status() {
      const info = {
        ...
        workspaceBar: !!document.querySelector(SEL.bar),
        button: !!document.getElementById(IDS.btn),
